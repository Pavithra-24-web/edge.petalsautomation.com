...
def _maybe_adapt_cls_calibration(
    ...
    tp_floor_multiplier: "float | None" = None,
    tp_fp_gap_floor: "float | None" = None,
    fp_drift_multiplier: "float | None" = None,
    val_cls_pos: "float | None" = None,
    val_cls_neg: "float | None" = None,
    tp_fp_gap_warn: "float | None" = None,
    fp_drift_warn_multiplier: "float | None" = None,
) -> Tuple[dict, bool, Optional[str]]:
    """
    Classifier-calibration adaptation driven by the real-inference safety
    eval report.

    Distinct from ``_maybe_adapt_training_balance``:
      • Reads the post-NMS / GT-matched ``safety_report`` rather than the
        per-anchor val_cls / val_box_dfl loss values.  This is the same
        signal Layer 1 uses to *gate* checkpoint acceptance; here it is
        used to *change training* by increasing ``w_cls``.
    ...
    Adaptation triggers (any one, evaluated independently):
      • tp_score_mean < tp_floor_multiplier × prior_prob
            (positives stuck near the head's initialisation bias —
             the head is not separating from background at all).
      • precision      < _CKPT_SAFETY_MIN_PRECISION
            (FP-flood signature).
      • preds_per_img  > _CKPT_SAFETY_MAX_PREDS_PER_IMAGE
            (FP-flood signature — derived from total_predicted_cells /
             evaluated_images, both stamped into the safety report by
             ``evaluate_yolo_pro_detection``).
      • map50          < _CKPT_SAFETY_MIN_MAP50  (only if finite — the
            eval may report None for degenerate sets with no GT).
      • tp_score_mean - fp_score_mean < tp_fp_gap_floor
        AND fp_score_mean > fp_drift_multiplier × prior_prob
            (rising-FP-confidence calibration drift — leading indicator
             before precision collapses or ranking inverts; both
             conditions required to avoid firing on healthy mid-training
             reports where the gap is small only because tp and fp are
             both still near the prior).

    Each metric silently no-ops when its value is missing or non-finite
    so the adapter cannot fire on absence of signal.

    Trigger metrics are NOT used as checkpoint ranking metrics — only as
    safety/adaptation signals.  The configured monitor (val_box_dfl /
    ...
    if tp_floor_multiplier is None:
        tp_floor_multiplier = _CLS_CAL_TP_FLOOR_MULT
    if tp_fp_gap_floor is None:
        tp_fp_gap_floor = _CLS_CAL_TP_FP_GAP_FLOOR
    if fp_drift_multiplier is None:
        fp_drift_multiplier = _CLS_CAL_FP_DRIFT_MULT
    if tp_fp_gap_warn is None:
        tp_fp_gap_warn = _CLS_CAL_TP_FP_GAP_WARN
    if fp_drift_warn_multiplier is None:
        fp_drift_warn_multiplier = _CLS_CAL_FP_DRIFT_WARN_MULT

    # Warmup guard — the safety report is noisy while TAL bootstraps.
    ...
    # Extract trigger metrics with strict missing/non-finite handling.
    ...
    # 1. tp_score_mean against prior-prob floor.
    if _is_finite_number(tp) and _is_finite_number(prior_prob) and float(prior_prob) > 0:
        tp_floor = float(prior_prob) * float(tp_floor_multiplier)
        if float(tp) < tp_floor:
            triggers.append(
                f"tp_score_mean={float(tp):.4f} < {tp_floor_multiplier:.1f}×prior_prob="
                f"{tp_floor:.4f}"
            )

    # 2. precision floor.
    if _is_finite_number(prec) and float(prec) < _CKPT_SAFETY_MIN_PRECISION:
        triggers.append(
            f"precision={float(prec):.4f} < {_CKPT_SAFETY_MIN_PRECISION:.4f}"
        )

    # 3. preds_per_image ceiling.
    preds_per_image: "float | None" = None
    if (
        _is_finite_number(total_preds)
        and _is_finite_number(n_images)
    ...
    # 4. map50 floor (only when finite — eval may report None on degenerate sets).
    if _is_finite_number(map50) and float(map50) < _CKPT_SAFETY_MIN_MAP50:
        triggers.append(
            f"map50={float(map50):.4f} < {_CKPT_SAFETY_MIN_MAP50:.4f}"
        )

    # 5. tp_fp_gap collapse + fp drift — the rising-FP-confidence calibration
    # drift mode.  Both conditions required:
    #   • gap = tp - fp < tp_fp_gap_floor  (ranking has narrowed)
    #   • fp_score_mean > fp_drift_multiplier × prior_prob
    #     (FP has actually drifted upward, not just stayed near the prior)
    # The gap is computed from tp and fp directly so this trigger works
    # regardless of whether the safety report carries `tp_fp_score_gap`.
    tp_fp_gap: "float | None" = None
    if (
        _is_finite_number(tp)
        and _is_finite_number(fp)
        and _is_finite_number(prior_prob)
        and float(prior_prob) > 0
    ):
        tp_fp_gap = float(tp) - float(fp)
        fp_drift_floor = float(prior_prob) * float(fp_drift_multiplier)
        if (
            tp_fp_gap < float(tp_fp_gap_floor)
            and float(fp) > fp_drift_floor
        ):
            triggers.append(
                f"tp_fp_gap={tp_fp_gap:.4f} < {float(tp_fp_gap_floor):.4f} "
                f"with fp_score_mean={float(fp):.4f} > "
                f"{float(fp_drift_multiplier):.1f}×prior_prob={fp_drift_floor:.4f}"
            )

    # 6. Early gap-warning trigger — fires *before* trigger #5 (hard drift)
    # when calibration is degrading but the gap has not yet collapsed.
    # All four conditions required:
    #   • tp_fp_gap < _CLS_CAL_TP_FP_GAP_WARN (warning band; above hard floor)
    #   • fp_score_mean > _CLS_CAL_FP_DRIFT_WARN_MULT × prior_prob (stricter
    #     FP-drift evidence than the hard trigger)
    #   • val_cls_neg > val_cls_pos (loss-side confirmation the negative
    #     branch is the one degrading, not generic mid-training noise)
    #   • preds_per_image known AND <= safety ceiling (excludes flood
    #     patterns — those continue to be handled by trigger #3 instead)
    # Skipped silently when any input is missing or non-finite so the
    # trigger cannot fire on absence of signal.
    if (
        _is_finite_number(tp)
        and _is_finite_number(fp)
        and _is_finite_number(prior_prob)
        and float(prior_prob) > 0
        and _is_finite_number(val_cls_pos)
        and _is_finite_number(val_cls_neg)
        and preds_per_image is not None
        and preds_per_image <= _CKPT_SAFETY_MAX_PREDS_PER_IMAGE
    ):
        if tp_fp_gap is None:
            tp_fp_gap = float(tp) - float(fp)
        fp_drift_warn_floor = float(prior_prob) * float(fp_drift_warn_multiplier)
        if (
            tp_fp_gap < float(tp_fp_gap_warn)
            and float(fp) > fp_drift_warn_floor
            and float(val_cls_neg) > float(val_cls_pos)
        ):
            triggers.append(
                f"early_gap_warning: tp_fp_gap={tp_fp_gap:.4f} < "
                f"{float(tp_fp_gap_warn):.4f} with fp_score_mean={float(fp):.4f} > "
                f"{float(fp_drift_warn_multiplier):.1f}×prior_prob="
                f"{fp_drift_warn_floor:.4f} and val_cls_neg={float(val_cls_neg):.4f} > "
                f"val_cls_pos={float(val_cls_pos):.4f} "
                f"(preds_per_image={preds_per_image:.2f} <= "
                f"{_CKPT_SAFETY_MAX_PREDS_PER_IMAGE:.2f})"
            )
    ...
    # Format every required field for the structured log line.
    ...
    ppi_str = (
        f"{preds_per_image:.2f}" if preds_per_image is not None else "None"
    )
    gap_str = f"{tp_fp_gap:.4f}" if tp_fp_gap is not None else "None"

    log_reason = (
        f"classifier calibration adaptation at epoch {epoch + 1}: "
        f"w_cls {current_w_cls:.2f}→{new_w_cls:.2f} (cap={float(w_cls_cap):.2f}); "
        f"tp_score_mean={_fmt(tp)}, fp_score_mean={_fmt(fp)}, "
        f"tp_fp_gap={gap_str}, "
        f"val_cls_pos={_fmt(val_cls_pos)}, val_cls_neg={_fmt(val_cls_neg)}, "
        f"precision={_fmt(prec)}, preds_per_image={ppi_str}, "
        f"map50={_fmt(map50)}; trigger_reasons=[{'; '.join(triggers)}]"
    )
...
def _compute_stabilization_policy(
    ...
    Patience (based on eff_epochs, never so small it cancels the budget):
      ...
      else     → max(8,  int(0.20 * eff_epochs))
    ...
    if n_train <= 0:
        return {
            ...
            "patience":             max(5, int(0.15 * req_ep)),
...
# Per-candidate-epoch adaptation that increases the classification loss
# weight (``w_cls``) when the real-inference safety eval reveals classifier
# failure — flat positive scores, FP-flood, low precision / mAP50.  This is
# the only mechanism in the loop that reads the safety report to *change
# training*, not just to gate checkpoint acceptance.  Triggers are
# evaluated independently and any single one is sufficient; missing or
# non-finite metrics never trigger.  May re-fire across epochs until
# w_cls reaches the cap.
#
# Triggers (shared semantics with Layer 1 safety gates where applicable):
#   • tp_score_mean < CLS_CAL_TP_FLOOR_MULT × prior_prob   (head stuck near prior)
#   • precision     < _CKPT_SAFETY_MIN_PRECISION           (FP-flood)
#   • preds_per_img > _CKPT_SAFETY_MAX_PREDS_PER_IMAGE     (FP-flood)
#   • map50         < _CKPT_SAFETY_MIN_MAP50    (only if finite — eval may
#                                                report None on degenerate sets)
#
# A bump step of 0.5 is intentionally larger than the legacy
# ``_maybe_adapt_training_balance`` step of 0.15: by the time the safety
# eval reports flat cls separation the legacy bump has either fired and
# failed or never fired at all, so the new mechanism needs to move
# meaningfully.  Cap of 2.0 keeps box/dfl as the dominant gradient share
# (box=2.0 + dfl=1.5 = 3.5 vs cls=2.0 → cls ≤ 36 % of the weighted sum).
_CLS_CAL_TP_FLOOR_MULT: float = 5.0
_CLS_CAL_W_CLS_STEP:    float = 0.5
_CLS_CAL_W_CLS_CAP:     float = 2.0

# ── Rising-FP-confidence (tp_fp_gap collapse) trigger ───────────────────────
# Targets the observed calibration drift mode where val_box_dfl keeps
# improving, preds/image stays controlled, and precision still slowly
# collapses because fp_score_mean drifts upward over epochs.  None of the
# four legacy triggers catches this until it has already turned into a
# precision collapse:
#   • TP-floor: tp_score_mean is fine; box learning is working.
#   • precision: only fires AFTER ranking has already inverted enough to
#     destroy AP — too late to recover the candidate epoch.
#   • preds/img: by assumption already capped — never trips.
#   • map50: only triggers after total collapse.
#
# The leading indicator is the gap itself: ``tp_score_mean - fp_score_mean``
# shrinks before precision crashes.  We trigger when the gap is below
# ``_CLS_CAL_TP_FP_GAP_FLOOR`` AND fp_score_mean has drifted above
# ``_CLS_CAL_FP_DRIFT_MULT × prior_prob`` — both conditions required so the
# trigger does NOT fire on healthy mid-training reports where the gap is
# small only because both tp and fp are still near the prior.
#
# Ranking semantics preserved:
#   • The hard ranking gate ``tp > fp`` continues to reject collapsed
#     checkpoints.  This trigger acts on a *narrow-but-still-positive*
#     gap, before the hard gate would reject — so it adapts training
#     rather than waiting for ranking inversion.
#   • The gap is a safety / adaptation signal, NOT a checkpoint ranking
#     metric.  The configured monitor (val_box_dfl / val_loss) still
#     selects checkpoints unchanged.
_CLS_CAL_TP_FP_GAP_FLOOR: float = 0.10
_CLS_CAL_FP_DRIFT_MULT:   float = 5.0

# ── Early gap-warning trigger ────────────────────────────────────────────────
# Preventive trigger that fires *before* the hard drift trigger above.
# Targets the observed EP4→EP5 collapse pattern: gap is still positive but
# already dropping, fp_score_mean has drifted clearly above the prior, the
# loss-side negative branch (val_cls_neg) is outpacing the positive branch
# (val_cls_pos), and preds_per_image is still controlled — so it is a
# calibration drift, not an FP flood.
#
# Bounds are intentionally tighter than the hard drift trigger:
#   • gap floor = 0.16 (warning band; above the 0.10 hard floor)
#   • fp drift  = 10× prior_prob (stricter than the 5× hard threshold,
#     so the warning trigger requires the FP head to have drifted further
#     before firing in the wider gap band)
#   • val_cls_neg > val_cls_pos (loss-side confirmation that the failing
#     branch is the negative one, not generic mid-training noise)
#   • preds_per_image known AND <= ceiling (excludes flood patterns —
#     those continue to be handled by the existing preds/image trigger)
#
# Ranking semantics preserved: same contract as every other trigger —
# only mutates w_cls and the diagnostic preset string.
_CLS_CAL_TP_FP_GAP_WARN:     float = 0.16
_CLS_CAL_FP_DRIFT_WARN_MULT: float = 10.0
...
def _export_candidate_rank_key(entry: dict, index: int) -> "tuple":
    ...
    The returned tuple is also used by ``_select_buffer_eviction_index``
    to keep the highest-ranked mosaic-on stability rescue safe from
    mosaic-OFF eviction.
    """
...
def _select_buffer_eviction_index(buffer: List[dict]) -> int:
    """
    Choose which ring-buffer entry to evict when over capacity.

    Default: pop the oldest entry (index 0) — preserves the legacy
    FIFO behavior for monitor / strict-rescue saves.

    Exception: when the oldest entry is the highest-ranked
    ``safety_stability_rescue`` checkpoint that was saved during the
    mosaic-ON phase, prefer evicting the next-oldest non-protected
    entry instead.  This stops a stretch of mosaic-OFF saves (the
    'mosaic-OFF collapse' failure mode) from silently flushing the
    near-best mosaic-ON stability rescue out of the candidate pool.

    If every entry is protected, falls back to popping the oldest.
    """
    if not buffer:
        raise ValueError("_select_buffer_eviction_index requires a non-empty buffer")

    protected_idx: "int | None" = None
    best_key: "tuple | None" = None
    for i, entry in enumerate(buffer):
        if (
            entry.get("checkpoint_reason") == "safety_stability_rescue"
            and entry.get("mosaic_active") is True
        ):
            key = _export_candidate_rank_key(entry, i)
...
def run_yolo_pro_training(
    ...
    # Ring buffer of accepted best-checkpoint EMA snapshots, oldest → newest.
    # Phase 3: widened from 3 → 8.  Admission is ranked on the cheap probe
    # (Phase 1); the authoritative full eval that actually picks the shipped
    # checkpoint runs once per finalist at end of training (Phase 2).  The
    # residual risk is that the probe under-ranks the true best epoch and it
    # gets evicted before the full eval can re-rank it — retaining more
    # finalists shrinks that risk to negligible.  The cost is MEMORY, not
    # epoch time: each entry holds float32 copies of the EMA shadows
    # (trainable weights) + a BN/non-trainable snapshot, and the Model
    # Testing stage runs the full eval at most `cap` times (bounded).
    # Tunable: raise for more margin on noisy-probe runs; lower if per-entry
    # snapshot memory (see the estimate logged below) is a concern for a
    # larger backbone / higher input resolution.
    _BEST_EMA_BUFFER_CAP = 8
    best_ema_buffer: List[dict] = []

    # Estimate peak buffer memory (EMA shadows + BN snapshot) × cap so a
    # larger model / higher resolution surfaces the cost in the log, even
    # though at 96²/nano it is only a few MB.  Shadows cover the trainable
    # variables and the BN snapshot the non-trainable ones, so their combined
    # float32 footprint is ≈ total_params × 4 bytes per entry.
    try:
        _buf_bytes_per_entry = int(_total_p) * 4            # float32 shadows + BN snapshot
        _buf_peak_mb  = (_buf_bytes_per_entry * _BEST_EMA_BUFFER_CAP) / (1024 * 1024)
        _buf_entry_mb = _buf_bytes_per_entry / (1024 * 1024)
        _buf_mem_line = (
            f"[{job_id}] best-EMA buffer: cap={_BEST_EMA_BUFFER_CAP}  "
            f"~{_buf_entry_mb:.1f} MB/entry  ~{_buf_peak_mb:.1f} MB peak "
            f"(EMA shadows + BN snapshot × cap; total_params={int(_total_p):,})"
        )
        logger.info(_buf_mem_line)
        _yolo_log_lines.append(_buf_mem_line)
        # Guard: warn if the retained buffer would dominate host memory.  The
        # threshold is deliberately generous (nano/96² is a few MB); it only
        # trips for large backbones / high resolution where the cap should be
        # reconsidered against available RAM.
        _BUF_MEM_WARN_MB = 1024.0
        if _buf_peak_mb > _BUF_MEM_WARN_MB:
            _buf_warn = (
                f"[{job_id}] WARNING: best-EMA buffer peak ~{_buf_peak_mb:.0f} MB "
                f"exceeds {_BUF_MEM_WARN_MB:.0f} MB soft budget "
                f"(cap={_BEST_EMA_BUFFER_CAP}, total_params={int(_total_p):,}).  "
                f"Consider lowering _BEST_EMA_BUFFER_CAP for this model size."
            )
            logger.warning(_buf_warn)
            _yolo_log_lines.append(_buf_warn)
    except Exception as _buf_mem_exc:
        logger.debug(f"[{job_id}] buffer memory estimate skipped: {_buf_mem_exc}")

    # Best safety-eval mAP50 of any SAVED checkpoint (monitor save or
    # safety-quality rescue save).  Used as the ratchet baseline for the
    # rescue gate: a rescue save requires
    # ``current_safety_map50 >= best_safety_map50 + _CKPT_RESCUE_MIN_MAP50_DELTA``
    # — comparing against the best saved value (not the previous epoch's
    # value) prevents noisy local bumps from spamming the buffer.  None
    # until the first save with a finite safety map50.  Tracked alongside
    # the primary monitor's best_val_* — not used as a ranking metric.
    best_safety_map50: Optional[float] = None
    best_safety_epoch: Optional[int]   = None
    best_safety_reason: Optional[str]  = None

    # Audit trail of every monitor-improving candidate the safety gate rejected.
    ...
    # Append mosaic/augmentation setup lines now that _mosaic_n is known.
    _yolo_log_lines.append(
        f"Mosaic schedule: ON for first {_mosaic_n} epoch(s),"
        f" OFF for final {_nomosaic_n} epoch(s) (close-mosaic refinement)."
    )
    _yolo_log_lines.append(
    ...
    for epoch in range(effective_epochs):
        ...
        if epoch == 0:
            ...
            logger.info(_cm_reset_line)
            _yolo_log_lines.append(_cm_reset_line)
            logger.info(
        ...
        # ── Real-inference calibration / safety eval ──────────────────────
        # Runs on EVERY post-warmup validation epoch — not gated by
        # checkpoint-candidate status — so the classifier-calibration
        # adapter receives a fresh signal even when the monitor plateaus,
        # the monitor regresses, or the cls guard would reject the
        # candidate.  These are precisely the regimes where cls
        # calibration is most likely to drift and the per-candidate
        # gating used to leave the adapter silent.
        #
        # Checkpoint acceptance semantics are unchanged: the conjunction
        # _ckpt_monitor_improved AND _ckpt_cls_guard_passed AND
        # _ckpt_safety_passed (below) still determines saves.  The safety
        # report's `tp_score_mean > fp_score_mean` invariant and the
        # detector-quality gates (precision floor, mAP50 floor, preds-per-
        # image ceiling — see `_checkpoint_quality_gate_failures`) act
        # only as gates / adaptation signals — never as ranking metrics.
        # The probe is a CHEAP 32-image signal used only for calibration
        # adaptation and to decide whether the canonical full export-safety
        # eval is worth running this epoch.  Probe metrics never enter the
        # export buffer and never participate in save / ranking decisions.
        _probe_safety_report: "dict | None" = None
        # Phase 1: the per-epoch canonical full export-safety eval was
        # removed — admission now runs on the cheap probe and the authoritative
        # full eval is deferred to end of training (Phase 2).  This name is
        # kept declared (always None in-loop) so any remaining reference is a
        # safe no-op until Phase 2 repurposes the end-of-training report.
        _full_safety_report: "dict | None" = None
        _safety_report: "dict | None" = None
        _ckpt_safety_passed = True
        _past_warmup = epoch >= max(0, policy["warmup_epochs"])
        if _past_warmup and has_val_data:
            _ph0_probe_t0 = time.perf_counter()
            _live_backup_sg = ema.backup_weights(model)
            ema.apply(model)
            try:
                _probe_safety_report = _run_checkpoint_safety_eval(
                    model=model,
                    X_test=X_test,
                    boxes_test=boxes_test,
                    label_names=label_names,
                    ...
                    anchor_grids=anchor_grids,
                    H=H, W=W, reg_max=reg_max,
                    conf_threshold=_safety_eval_conf,
                    subset_size=_CHECKPOINT_SAFETY_EVAL_SUBSET,
                    job_id=job_id,
                    runtime_conf_threshold=_runtime_safety_conf,
                )
            except Exception as _sg_exc:
                logger.warning(
                    f"[{job_id}] EP{epoch+1} safety/calibration eval raised: "
                    f"{_sg_exc} — gate and adaptation are no-ops for this epoch."
                )
                _probe_safety_report = None
            finally:
                ema.restore(model, _live_backup_sg)
            _ph0_probe_s = time.perf_counter() - _ph0_probe_t0
            # Calibration adapter and probe-time decisions read the probe
            # report through the legacy ``_safety_report`` name — the full
            # eval below overwrites it on candidate epochs so downstream
            # save / ranking code keeps seeing the authoritative metrics.
            _safety_report = _probe_safety_report

            # ── Layer 2: classifier calibration adaptation ──────────────
            # Runs on every post-warmup val epoch, independent of candidate
            # status.  This is what makes cls-failure signals reach the
            # adapter even when the monitor plateaus / regresses or the
            # cls guard would reject the candidate.  The adapter's
            # internal warmup / missing-signal guards make it a no-op
            # when `_safety_report` is None (e.g. eval failed above) or
            # when the report shows healthy classifier separation.
        ...
        # Safety-gate decision uses the freshest report (or None → no-op
        # via `_checkpoint_safety_gate_passes`).  Evaluated unconditionally
        # so the gate result is consistent regardless of whether this
        # epoch is a candidate.  Combined with monitor + cls guard below
        # to keep checkpoint acceptance semantics unchanged.
        _ckpt_safety_passed = _checkpoint_safety_gate_passes(
            _safety_report,
            epoch=epoch,
            warmup_epochs=policy["warmup_epochs"],
        )
        _full_safety_eval_error: "str | None" = None

        # The cheap epoch probe above feeds calibration/adaptation. Before an
        # EMA snapshot is allowed into the export buffer, validate it on the
        # same full validation set used by post-training export evaluation.
        # This prevents subset luck from creating a buffer of snapshots that
        # later all fail during walk-back.
        _monitor_candidate_after_probe = (
            _ckpt_monitor_improved
            and _ckpt_cls_guard_passed
            and _ckpt_safety_passed
        )
        _rescue_candidate_after_probe = False
        if _ckpt_safety_passed and _safety_report is not None:
            _rescue_candidate_after_probe = _checkpoint_rescue_qualifies(
                safety_report=_safety_report,
                best_safety_map50=best_safety_map50,
                epoch=epoch,
                warmup_epochs=policy["warmup_epochs"],
            )
            if not _rescue_candidate_after_probe:
                # Cheap-probe pre-check for the near-best stability rescue.
                # We pay for a full safety eval if either the strict rescue
                # OR the stability rescue look plausible from the subset.
                _stab_ok_probe, _ = _checkpoint_stability_rescue_qualifies(
                    safety_report=_safety_report,
                    best_safety_map50=best_safety_map50,
                    epoch=epoch,
                    warmup_epochs=policy["warmup_epochs"],
                )
                _rescue_candidate_after_probe = _stab_ok_probe
        # ── Admission snapshot (Phase 1) ─────────────────────────────────
        # The cheap 32-image probe above is now the admission / ranking
        # signal.  The authoritative full export-safety eval no longer runs
        # per epoch — it is deferred to the SINGLE end-of-training eval
        # (Phase 2), which re-scores and re-ranks the buffered snapshots and
        # backfills their full_* metrics.  We still capture the non-trainable
        # (BatchNorm) model state under EMA-applied weights at admission time
        # so the buffered entry can be reconstructed bit-for-bit when the
        # end-of-training eval scores it.  Captured only for candidate epochs
        # (probe monitor / cls-guard / safety pass, or a probe-qualifying
        # rescue); remains None otherwise.
        _admission_non_trainable_snapshot: "list | None" = None
        if (
            _past_warmup
            and has_val_data
            and _safety_report is not None
            and (_monitor_candidate_after_probe or _rescue_candidate_after_probe)
        ):
            _live_backup_full_sg = ema.backup_weights(model)
            ema.apply(model)
            try:
                # Snapshot non-trainable model state under EMA-applied
                # weights so the end-of-training restore re-creates the exact
                # admission model (not (EMA-N trainables, latest-epoch BN
                # stats)).  Cheap; the only cost that remains in-loop.
                _ph0_snap_t0 = time.perf_counter()
                _admission_non_trainable_snapshot = _snapshot_non_trainable_state(model)
                _ph0_snapshot_s = time.perf_counter() - _ph0_snap_t0
            except Exception as _snap_exc:
                # Snapshot failure is non-fatal: the entry is still admitted
                # on the probe signal, and the end-of-training restore falls
                # back to live non-trainable state (helper is a no-op then).
                _admission_non_trainable_snapshot = None
                _snap_fail_line = (
                    f"[{job_id}] EP{epoch+1} admission BN-snapshot failed: "
                    f"{_snap_exc} — admitting on probe with live-BN fallback."
                )
                logger.warning(_snap_fail_line)
                _yolo_log_lines.append(_snap_fail_line)
            finally:
                ema.restore(model, _live_backup_full_sg)
        ...
        _tr_sr = _safety_report or {}
        _tr_tp = _tr_sr.get("tp_score_mean")
        _tr_fp = _tr_sr.get("fp_score_mean")
        _tr_gap = (
            float(_tr_tp) - float(_tr_fp)
            if isinstance(_tr_tp, (int, float)) and isinstance(_tr_fp, (int, float))
            and math.isfinite(float(_tr_tp)) and math.isfinite(float(_tr_fp))
            else None
        )
        _tr_total_preds = _tr_sr.get("total_predicted_cells")
        _tr_n_imgs      = _tr_sr.get("evaluated_images")
        _tr_ppi: Optional[float] = None
        if (
            isinstance(_tr_total_preds, (int, float))
            and isinstance(_tr_n_imgs, (int, float))
            and math.isfinite(float(_tr_total_preds))
            and _tr_n_imgs is not None
            and float(_tr_n_imgs) > 0
        ):
            _tr_ppi = float(_tr_total_preds) / float(_tr_n_imgs)
        _calibration_trace.append({
            "epoch":           epoch + 1,
            "tp_score_mean":   _tr_tp,
            "fp_score_mean":   _tr_fp,
            "tp_fp_gap":       _tr_gap,
            "precision":       _tr_sr.get("precision"),
            "map50":           _tr_sr.get("map50"),
            "preds_per_image": _tr_ppi,
        ...
        _ckpt_guard_passed = _ckpt_cls_guard_passed and _ckpt_safety_passed
        _save_ckpt = _ckpt_monitor_improved and _ckpt_guard_passed
        _ckpt_guard_rejected = _ckpt_monitor_improved and not _ckpt_guard_passed
        _checkpoint_reason: Optional[str] = "monitor" if _save_ckpt else None

        # ── Safety-quality rescue checkpoint ─────────────────────────────
        # Complements the primary monitor save: a safety-passing epoch
        # whose real-inference map50 has meaningfully improved is saved
        # even when the primary monitor (val_box_dfl / val_loss) has
        # stalled or regressed.  The primary monitor is preserved as the
        # ranking metric — rescue only adds a second save path, never
        # changes how monitor-improving safe checkpoints are accepted.
        _save_rescue = False
        _prev_best_safety_map50 = best_safety_map50
        # Phase 1: rescue / stability decisions read the cheap PROBE report
        # (the in-loop admission signal).  The per-epoch full eval is gone;
        # the authoritative end-of-training eval (Phase 2) re-ranks whatever
        # the probe admitted.  ``_safety_report`` here is the probe report
        # (no longer overwritten by a full eval); None only when the probe
        # itself did not run / failed, in which case neither rescue fires.
        if (
            (not _save_ckpt)
            and _ckpt_safety_passed
            and _safety_report is not None
        ):
            _save_rescue = _checkpoint_rescue_qualifies(
                safety_report=_safety_report,
                best_safety_map50=best_safety_map50,
                epoch=epoch,
                warmup_epochs=policy["warmup_epochs"],
        ...
        if _save_rescue:
            _save_ckpt = True
            # Rescue supersedes the monitor-rejection audit on this epoch:
            # the candidate WAS saved, just via a different path, so it
            # should not also appear in the rejected-candidates list.
            _ckpt_guard_rejected = False
            _checkpoint_reason = "safety_quality_rescue"

        # ── Safety-stability rescue checkpoint ───────────────────────────
        # Companion to the strict rescue above: saves near-best safety
        # checkpoints (current_safety_map50 within 0.01 of best saved)
        # whose real-detector quality is strong (precision >= 0.20,
        # tp_fp_gap >= 0.10, runtime_preds_per_image <= 20).  Independent
        # of val_box_dfl improvement.  Only fires when neither the monitor
        # path NOR the strict rescue path is already saving this epoch.
        _save_stability_rescue = False
        _stability_skip_reasons: List[str] = []
        if (
            (not _save_ckpt)
            and _ckpt_safety_passed
            and _safety_report is not None
        ):
            _save_stability_rescue, _stability_skip_reasons = (
                _checkpoint_stability_rescue_qualifies(
                    safety_report=_safety_report,
                    best_safety_map50=best_safety_map50,
                    epoch=epoch,
                    warmup_epochs=policy["warmup_epochs"],
        ...
        if _save_stability_rescue:
            _save_ckpt = True
            _ckpt_guard_rejected = False
            _checkpoint_reason = "safety_stability_rescue"
        elif (
            _ckpt_safety_passed
            and _safety_report is not None
            and _stability_skip_reasons
            and not _save_ckpt
        ):
            # Composite safety passed but stability rescue declined.  Log
            # exactly which gates rejected the save so operators can audit
            # near-miss epochs without scraping per-epoch traces.
            _skip_filtered = [
                r for r in _stability_skip_reasons
                if r not in ("epoch_within_warmup", "no_safety_report",
                             "composite_safety_gate_failed")
            ]
            if _skip_filtered:
                _stab_skip_line = (
                    f"[{job_id}] safety stability rescue skipped at epoch "
                    f"{epoch+1}: {'; '.join(_skip_filtered)}"
                )
                logger.info(_stab_skip_line)
                _yolo_log_lines.append(_stab_skip_line)

        if _ckpt_guard_rejected:
            _sr = _safety_report or {}
            _sr_failures = _checkpoint_quality_gate_failures(_sr)
            if _full_safety_eval_error:
                _sr_failures.append(
                    f"full_safety_eval_error: {_full_safety_eval_error}"
                )
            if not _ckpt_safety_passed:
                _failure_str = "; ".join(_sr_failures) if _sr_failures else (
                    "ranking invariant only — no quality-gate detail"
                )
                logger.warning(
                    f"[{job_id}] EP{epoch+1} checkpoint rejected by real-inference "
                    f"safety gate — {ckpt_metric_name} improved to "
                    f"{monitor_loss:.4f} but safety eval failed: {_failure_str}.  "
                    f"Patience held to allow recovery."
                )
            else:
                logger.warning(
                    ...
                    f"ranking collapse (tp_score_mean < fp_score_mean).  "
                    f"Patience held to allow recovery."
                )
            # Audit the rejection so the end-of-training summary can name
            # exactly which gates failed, with which values, on each epoch.
            _reject_reasons: List[str] = list(_sr_failures)
            if _ckpt_safety_passed and not _ckpt_cls_guard_passed:
            ...
            _preds_per_image_val: "float | None" = None
            _tp_n = _sr.get("total_predicted_cells")
            _ni_n = _sr.get("evaluated_images")
            if (
                isinstance(_tp_n, (int, float)) and isinstance(_ni_n, (int, float))
                and math.isfinite(float(_tp_n)) and _ni_n is not None
                and float(_ni_n) > 0
            ):
                _preds_per_image_val = float(_tp_n) / float(_ni_n)
            # Runtime preds per image — the metric the flood gate actually
            # consumes.  Tracked alongside the legacy AP-threshold value so
            # the rejection summary can name the right number.
            _rt_tp_n = _sr.get("runtime_total_predicted_cells")
            _runtime_preds_per_image_val: "float | None" = None
            if (
                isinstance(_rt_tp_n, (int, float))
                and math.isfinite(float(_rt_tp_n))
                and isinstance(_ni_n, (int, float))
                and _ni_n is not None
                and float(_ni_n) > 0
            ):
                _runtime_preds_per_image_val = float(_rt_tp_n) / float(_ni_n)
            _rejected_ckpt_candidates.append({
                ...
                "failed_gates":     _reject_reasons,
                "val_cls":          mean_val_cls,
                "saved_ckpt_val_cls": saved_ckpt_val_cls,
                "safety_metrics": {
                    "tp_score_mean":  _sr.get("tp_score_mean"),
                    "fp_score_mean":  _sr.get("fp_score_mean"),
                    "precision":      _sr.get("precision"),
                    "map50":          _sr.get("map50"),
                    "preds_per_image": _preds_per_image_val,
                    "runtime_preds_per_image": _runtime_preds_per_image_val,
                    "runtime_conf_threshold":  _sr.get("runtime_conf_threshold"),
                },
            })
        # Phase 1: admission is on the cheap probe signal — the per-epoch
        # full export-safety eval is gone, so there is no longer a hard
        # "full eval must have run" admission invariant here.  The
        # authoritative full eval runs ONCE at end of training (Phase 2)
        # and re-ranks / walks back whatever the probe admitted.  Until
        # then, buffered entries carry probe metrics; their full_* fields
        # stay null (backfilled by the end-of-training eval).
        if _save_ckpt:
            # Primary monitor bookkeeping — only updated for monitor saves.
            # Rescue saves intentionally do NOT advance the primary monitor's
            # best value: the monitor metric still rules monitor saves, so
            # its low-water mark must not absorb the rescue's monitor value.
            if (
                _checkpoint_reason != "safety_quality_rescue"
                and _checkpoint_reason != "safety_stability_rescue"
            ):
            ...
            # Append to bounded ring buffer (newest at end).  Walk-back on
            # final-eval collapse iterates this newest → oldest.
            #
            # Phase 1: the cheap 32-image PROBE is the admission / ranking
            # score recorded here (probe_* fields, and the legacy generic
            # ``safety_*`` fields the in-loop ratchet / eviction read).  The
            # canonical full export-safety metrics (``full_*``) are DEFERRED
            # to the single end-of-training eval (Phase 2), which backfills
            # them and re-ranks; they are left present-but-null so downstream
            # readers and tests keep the field names.
            _probe_sr = _probe_safety_report or {}
            _probe_map50     = _probe_sr.get("map50")
            _probe_precision = _probe_sr.get("precision")
            _probe_tp        = _probe_sr.get("tp_score_mean")
            _probe_fp        = _probe_sr.get("fp_score_mean")
            _probe_tp_fp_gap = (
                float(_probe_tp) - float(_probe_fp)
                if isinstance(_probe_tp, (int, float))
                and isinstance(_probe_fp, (int, float))
                and math.isfinite(float(_probe_tp))
                and math.isfinite(float(_probe_fp))
                else None
            )
            _probe_runtime_ppi = _extract_runtime_preds_per_image(_probe_sr)
            # Phase 1: the in-loop admission / ranking score IS the probe.
            # Alias the generic ``_sr_*`` names the ratchet and rescue log
            # lines below read so they operate on the probe metrics.  The
            # canonical full metrics are backfilled at end of training.
            _sr_map50       = _probe_map50
            _sr_precision   = _probe_precision
            _sr_tp_fp_gap   = _probe_tp_fp_gap
            _sr_runtime_ppi = _probe_runtime_ppi

            def _ffin(v: object) -> "float | None":
                if isinstance(v, (int, float)) and math.isfinite(float(v)):
                    return float(v)
                return None

            best_ema_buffer.append({
                "epoch":            epoch + 1,
                "ema_shadows":      [tf.identity(s) for s in ema._shadows],
                # Snapshot of non-trainable model state (BatchNorm moving
                # stats etc.) at admission time.  Restored alongside the
                # EMA shadows at final / walk-back so the model evaluated at
                # export is bit-for-bit the admission model (no BN drift
                # between admission and the end-of-training eval).
                "non_trainable_snapshot": _admission_non_trainable_snapshot,
                "ckpt_metric_name": ckpt_metric_name,
                "monitor_metric":   ckpt_metric_name,
                "monitor_value":    monitor_loss,
                "val_box_dfl":      mean_val_box_dfl,
                "val_loss":         mean_val_loss,
                "val_cls":          mean_val_cls,
                # Legacy generic field names — Phase 1 populates these from
                # the cheap probe (the in-loop admission / ranking score).
                # The end-of-training full eval fills the ``full_*`` fields
                # below; it does not overwrite these.
                "safety_tp_score_mean": _probe_tp,
                "safety_fp_score_mean": _probe_fp,
                "checkpoint_reason":      _checkpoint_reason or "monitor",
                "safety_map50":            _ffin(_probe_map50),
                "safety_precision":        _ffin(_probe_precision),
                "safety_tp_fp_gap":        _probe_tp_fp_gap,
                "runtime_preds_per_image": _probe_runtime_ppi,
                "runtime_conf_threshold":  _probe_sr.get("runtime_conf_threshold"),
                # Canonical full export-safety metrics — DEFERRED to the
                # end-of-training eval (Phase 2).  Present-but-null here so
                # downstream readers / tests keep the field names; the final
                # eval backfills them and becomes the ranking source of truth.
                ...
                # Probe metrics — the admission / ranking signal in Phase 1.
                "probe_map50":            _ffin(_probe_map50),
                "probe_precision":        _ffin(_probe_precision),
                "probe_tp_fp_gap":        _probe_tp_fp_gap,
                "probe_evaluated_images": _probe_sr.get("evaluated_images"),
                # Mosaic-phase tag used by ``_select_buffer_eviction_index``
                # to protect the near-best mosaic-ON stability rescue from
                # eviction by a burst of mosaic-OFF saves.
                "mosaic_active":          bool(epoch < train_cfg.mosaic_epochs),
            })
            if len(best_ema_buffer) > _BEST_EMA_BUFFER_CAP:
                best_ema_buffer.pop(_select_buffer_eviction_index(best_ema_buffer))
            # Save-time log line that surfaces BOTH probe and full metrics
            # so a future probe/full mismatch is auditable from the buffer
            # log alone — not just from the dedicated divergence line.
            ...
            _save_summary_line = (
                ...
                f"probe(map50={_fmt(_probe_map50)} "
                f"prec={_fmt(_probe_precision)} "
                f"gap={_fmt(_probe_tp_fp_gap)} "
                f"runtime_ppi={_fmt(_probe_runtime_ppi, '.2f')})  "
                f"full(deferred to end-of-training eval)"
            )
            logger.info(_save_summary_line)
            _yolo_log_lines.append(_save_summary_line)
            # Track best saved safety map50 separately from the primary
            # monitor so the rescue ratchet is not displaced by monitor
            # saves whose safety map50 is missing or lower than a prior
            # rescue's.
            if (
                isinstance(_sr_map50, (int, float))
                and math.isfinite(float(_sr_map50))
                and (
                    best_safety_map50 is None
                    or float(_sr_map50) > float(best_safety_map50)
                )
            ):
                best_safety_map50  = float(_sr_map50)
                best_safety_epoch  = epoch + 1
                best_safety_reason = _checkpoint_reason or "monitor"
            ...
            patience_count = 0
            # Dedicated rescue-save log line — operators grep for the
            # exact tag '★ckpt-rescue' to locate epochs that were saved
            # via the safety-quality path rather than the monitor path.
            if _checkpoint_reason == "safety_quality_rescue":
                _rescue_prev_str = (
                    f"{float(_prev_best_safety_map50):.4f}"
                    if _prev_best_safety_map50 is not None
                    and isinstance(_prev_best_safety_map50, (int, float))
                    and math.isfinite(float(_prev_best_safety_map50))
                    else "None"
                )
                _rescue_new_str = (
                    f"{float(_sr_map50):.4f}"
                    if isinstance(_sr_map50, (int, float))
                    and math.isfinite(float(_sr_map50))
                    else "None"
                )
                _rescue_gap_str = (
                    f"{float(_sr_tp_fp_gap):.4f}"
                    if _sr_tp_fp_gap is not None else "None"
                )
                _rescue_prec_str = (
                    f"{float(_sr_precision):.4f}"
                    if isinstance(_sr_precision, (int, float))
                    and math.isfinite(float(_sr_precision))
                    else "None"
                )
                _rescue_ppi_str = (
                    f"{float(_sr_runtime_ppi):.2f}"
                    if _sr_runtime_ppi is not None else "None"
                )
                _rescue_monitor_str = (
                    f"{float(monitor_loss):.4f}"
                    if math.isfinite(monitor_loss) else "nan"
                )
                _rescue_log_line = (
                    f"[{job_id}] ★ckpt-rescue EP{epoch+1}  "
                    f"prev_safety_map50={_rescue_prev_str}  "
                    f"new_safety_map50={_rescue_new_str}  "
                    f"monitor={ckpt_metric_name}={_rescue_monitor_str}  "
                    f"tp_fp_gap={_rescue_gap_str}  "
                    f"precision={_rescue_prec_str}  "
                    f"runtime_preds_per_image={_rescue_ppi_str}"
                )
                logger.info(_rescue_log_line)
                _yolo_log_lines.append(_rescue_log_line)
            elif _checkpoint_reason == "safety_stability_rescue":
                _stab_best_str = (
                    f"{float(_prev_best_safety_map50):.4f}"
                    if _prev_best_safety_map50 is not None
                    and isinstance(_prev_best_safety_map50, (int, float))
                ...
                _stab_delta_str = (
                    f"{float(_sr_map50) - float(_prev_best_safety_map50):+.4f}"
                    if (
                        isinstance(_sr_map50, (int, float))
                        and math.isfinite(float(_sr_map50))
                        and _prev_best_safety_map50 is not None
                        and isinstance(_prev_best_safety_map50, (int, float))
                        and math.isfinite(float(_prev_best_safety_map50))
                    )
                    else "None"
                ...
                _stab_rt_thr = _probe_sr.get("runtime_conf_threshold")
                _stab_thr_str = (
                    f"{float(_stab_rt_thr):.4f}"
                    if isinstance(_stab_rt_thr, (int, float))
                    and math.isfinite(float(_stab_rt_thr))
                    else "None"
                )
                _stab_log_line = (
                    f"[{job_id}] ★ckpt-stability safety stability rescue "
                    f"checkpoint at epoch {epoch+1}  "
                    f"current_safety_map50={_stab_cur_str}  "
                    f"best_saved_safety_map50={_stab_best_str}  "
                    f"map50_delta_from_best={_stab_delta_str}  "
                    f"precision={_stab_prec_str}  "
                    f"tp_fp_gap={_stab_gap_str}  "
                    f"runtime_preds_per_image={_stab_ppi_str}  "
                    f"runtime_conf_threshold={_stab_thr_str}"
                )
                logger.info(_stab_log_line)
                _yolo_log_lines.append(_stab_log_line)
        ...
        # '★ckpt' marks monitor-improving saves, '★ckpt-rescue' marks
        # safety-quality rescue saves — helps operators distinguish the
        # two save paths in long training logs without scraping the
        # dedicated rescue log line above.
        if _checkpoint_reason == "safety_quality_rescue":
            _ckpt_star = " ★ckpt-rescue"
        elif _checkpoint_reason == "safety_stability_rescue":
            _ckpt_star = " ★ckpt-stability"
        elif _save_ckpt:
            _ckpt_star = " ★ckpt"
        else:
            _ckpt_star = ""
        ...
        # `is_candidate` is exactly the condition under which the canonical
        # full export-safety eval ran this epoch (i.e. full_eval_s > 0 iff
        # this is True, barring an instant raise).  Logged and persisted so
        # the baseline pathology can be quantified without scraping logs.
        _ph0_is_candidate = bool(
            _past_warmup
            and has_val_data
            and _safety_report is not None
            and (_monitor_candidate_after_probe or _rescue_candidate_after_probe)
        ...
        # the model can refine on the un-augmented distribution.  Checkpoint
        # saves (monitor / safety_quality_rescue / safety_stability_rescue)
        # continue normally — only the early-stop exit is gated.
    ...
    # Bounded cost: len(best_ema_buffer) full evals total — never per-epoch.
    _raise_if_cancelled(job_id)
    ...
    if not best_ema_buffer:
        # No monitor-improving candidate ever passed the composite safety
        # gate.  Exporting the live EMA weights here would ship a model whose
        # classifier health was never validated by real inference — exactly
        # the silent failure the safety gates were added to prevent.  Fail
        # loudly with a structured summary instead.
    ...
    logger.info(
        ...
        f"{len(best_ema_buffer)} buffer finalist(s) … "
    ...
    # Evaluate every finalist once on the full validation set and backfill the
    # canonical ``full_*`` metrics + ranking_status onto its buffer entry.
    # ``_finalist_reports`` stays index-aligned with ``best_ema_buffer`` so the
    # selection / rejection logic below can recover each entry's eval report.
    _finalist_reports: List[dict] = []
    for _idx, _entry in enumerate(best_ema_buffer):
        _raise_if_cancelled(job_id)
        for shadow, best_s in zip(ema._shadows, _entry["ema_shadows"]):
            shadow.assign(best_s)
        ema.apply(model)
        # Restore the non-trainable state (BN moving stats, etc.) captured at
        # admission so each finalist is evaluated as the EXACT model the probe
        # admitted — no BN drift between admission and this end-of-training eval.
        _restore_non_trainable_state(model, _entry.get("non_trainable_snapshot"))
        try:
        ...
        def _fmt(v: object, spec: str = ".4f") -> str:
            ...
            return "None"
        _mt_line = (
            f"[{job_id}] Model Testing: finalist {_idx + 1}/{len(best_ema_buffer)} "
    ...
    _ok_indices = [
        i for i, _e in enumerate(best_ema_buffer)
    ...
    if not _ok_indices:
        ...
        for i, _e in enumerate(best_ema_buffer):
        ...
        _cause = (
            "final_eval_collapsed" if len(best_ema_buffer) <= 1
    ...
    logger.info(
        ...
        f"Selected from {len(_ok_indices)}/{len(best_ema_buffer)} ranking-ok finalist(s)."
    )
    _yolo_log_lines.append(
    ...
    # full ring-buffer contents (epoch + full metrics + ranking_status) so the
    # baseline for Phases 1-3 is archived per job in training_history.
    ...
    # Restore the selected finalist's EMA shadows + non-trainable state so the
    # exported .keras / .tflite artifacts reflect the EXACT model that was
    # evaluated and selected (the last finalist evaluated in the loop above may
    # not be the selected one).  ``eval_report`` was produced against precisely
    # this state, so the reported metrics match the exported weights.
    for shadow, best_s in zip(ema._shadows, _restored_entry["ema_shadows"]):
        shadow.assign(best_s)
    ema.apply(model)
    _restore_non_trainable_state(model, _restored_entry.get("non_trainable_snapshot"))
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            tflite_f32_bytes = export_tflite_float32(
                model=model, output_path=tflite_f32_path, input_shape=input_shape,
            )
        ...
        try:
            ...
            tflite_int8_bytes = export_tflite_int8(
                model=model, output_path=tflite_int8_path, input_shape=input_shape,
        ...
        try:
            ...
            tflite_decoded_bytes = export_tflite_float32_decoded(
                model=model, output_path=tflite_decoded_path,
