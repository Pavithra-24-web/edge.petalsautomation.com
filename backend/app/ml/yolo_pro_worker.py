...
# from app.ml.gpu_utils import worker_uses_gpu
...
#     # A test split of nothing but background images is a legitimate
#     # false-positive-rate test, not "no data": there is nothing to detect and
#     # the question is whether the model fires anyway. Only bail when there are
#     # no images at all, or when the split has neither ground truth nor a single
#     # image to measure false positives on.
#     _n_gt_images  = sum(1 for b in boxes_test if b)
#     _n_bg_images  = sum(1 for b in boxes_test if not b)
#     if len(X_test) == 0 or (_n_gt_images == 0 and _n_bg_images == 0):
...
#     if _n_gt_images == 0:
#         logger.info(
#             f"[{job_id}] YOLO-Pro eval: all {_n_bg_images} test images are "
#             "background — mAP/precision/recall are undefined, reporting "
#             "background_fp_rate only."
#         )
...
#     # An all-background test split has no ground truth, so every AP term is
#     # vacuously 0.0.  Reporting 0.0 would read as "the model detects nothing"
#     # when the correct reading is "there was nothing to detect".  Undefined is
#     # None; background_fp_rate below is the metric that actually has meaning
#     # for this split.  (The no_data return above already uses None for these,
#     # so callers handle the shape.)
#     if total_gt == 0:
#         map_coco = map50 = map75 = None
#         mean_prec = mean_rec = None
...
#     # contributes no AP term, so detections fired on it move the number only
#     # indirectly (through the shared precision denominator) and often not at
#     # all.  Without a dedicated metric a user who adds background images has no
#     # way to tell whether it helped.
#     #
#     # Measured at the runtime threshold when one is supplied — a deployed model
#     # applies that threshold, so a detection below it is not a false positive
#     # anyone will ever see.  Falls back to the (permissive) AP threshold, which
#     # over-reports; the reported threshold is carried alongside the number so
#     # the two are never compared across different thresholds.
#     _bg_thr = (
...
#     )
#     _bg_images = [_img for _img in per_image_data if not _img["gt_by_class"]]
#     _bg_fp_total = int(sum(
#         1
#         for _img in _bg_images
#         for _d in _img["dets"]
#         if float(_d["score"]) > _bg_thr
#     ))
#     _bg_images_with_fp = int(sum(
#         1
#         for _img in _bg_images
#         if any(float(_d["score"]) > _bg_thr for _d in _img["dets"])
#     ))
#     _bg_fp_rate = (
#         round(_bg_fp_total / len(_bg_images), 6) if _bg_images else None
#     )
#     if _bg_images:
#         logger.info(
#             f"[{job_id}] background FP rate: {_bg_fp_total} detections over "
#             f"{len(_bg_images)} background images = {_bg_fp_rate:.3f} per image "
#             f"({_bg_images_with_fp} image(s) fired at all) "
#             f"at conf > {_bg_thr:.4f}"
#         )
...
#         # Background ("negative") image false-positive reporting. None when the
#         # test split contains no background images.
#         "background_images":         int(len(_bg_images)),
#         "background_fp_rate":        _bg_fp_rate,
#         "background_images_with_fp": _bg_images_with_fp,
#         "background_fp_threshold":   round(_bg_thr, 6),
#         "gt_cells": total_gt,
...
#     # Denominator is the number of images that actually contain boxes, NOT the
#     # whole set.  `avg_boxes_per_image` feeds _resolve_training_balance_policy's
#     # preset thresholds (>= 2.0 selects the dense preset, >= 1.0 raises
#     # prior_prob from 0.01 to 0.02).  Counting background images in the
#     # denominator would drag the average down purely because negatives were
#     # added, silently flipping a dense dataset back to the sparse preset — a
#     # policy change with no change in the objects being detected.  The signal
#     # the presets want is "how crowded is a populated image", which is exactly
#     # boxes-per-*labeled*-image.
#     #
#     # With no negatives present labeled_images == num_images, so this is
#     # numerically identical to the previous formula.
#     avg_boxes_per_image = (float(total_boxes) / labeled_images) if labeled_images > 0 else 0.0
...
#         # Share of the training set that carries no boxes.  Surfaced as its own
#         # metric so the achieved negative ratio is visible in the training log
#         # next to the presets it influences, rather than having to be inferred.
#         "negative_image_fraction": round(1.0 - labeled_image_fraction, 6),
#     }
...
# # Abort training if this many RAW non-finite steps occur within one epoch.
# # "Raw" = measured on the head outputs BEFORE the loss sanitizers run
# # (``_acc_raw_nonfinite_count``).  The post-sanitiser ``total_loss`` is
# # unconditionally finite by construction — every loss term ends in a
# # ``tf.where(is_finite(x), x, 0)`` — so keying this abort on the loss value
# # made it dead code.  Why abort at all: the per-step gate skips the optimizer
# # + EMA update on a bad step, but BN moving stats are still updated inside the
# # ``training=True`` forward pass that produced the NaN.  Bounding the count
# # prevents the model degrading further while still tolerating rare transient
# # instability.
# _MAX_NAN_STEPS_PER_EPOCH: int = 3

# # Abort the epoch when this fraction of its steps were skipped as non-finite.
# # The old guard only fired when EVERY step was skipped (`_finite_count == 0`),
# # which let a run where 13 of 16 steps were skipped proceed to validation and
# # admit a poisoned checkpoint.  A model that cannot take three quarters of its
# # gradient steps is not training — it is decaying.
# _MAX_SKIPPED_STEP_RATIO_PER_EPOCH: float = 0.25

# # ── Mixed-precision dynamic loss scale ──────────────────────────────────────
# # Under a LossScaleOptimizer a "skipped step" usually means the scale
# # controller is doing its job, NOT that the model broke.  The optimizer starts
# # at ``initial_scale`` and every step whose scaled gradients overflow float16
# # is skipped (a true weight no-op) and halves the scale.  Starting at the Keras
# # default 2**15 therefore costs up to 15 consecutive skipped steps before the
# # first weight update can land — more steps than a small dataset's entire first
# # epoch.  Job 8718cdaf hit exactly that: 19/19 steps skipped with raw
# # non-finite head outputs = 0 (every forward pass clean, no weight ever
# # modified), which the skip-ratio abort misread as model corruption.
# #
# # Both halves of the fix live here:
# #   1. Start the scale low enough that calibration finishes inside the first
# #      epoch (`_resolve_initial_loss_scale`) while staying high enough that
# #      small fp16 gradients do not flush to zero, and let it grow back
# #      (`_loss_scale_growth_steps`).
# #   2. Never key an abort on the skip count alone under mixed precision —
# #      require an honest corruption signal.  See the epoch-end guards.
# #
# # Floor 2**8 = 256: fp16's smallest normal is ~6.1e-5, so a 1e-7 gradient
# # scaled by 256 is 2.6e-5 — still representable (subnormals reach ~6e-8), and
# # the dynamic controller doubles up from there.  Ceiling 2**15: the Keras
# # default, kept for datasets whose epochs are long enough to absorb the full
# # halving chain.
# _MIN_INITIAL_LOSS_SCALE_EXP: int = 8
# _MAX_INITIAL_LOSS_SCALE_EXP: int = 15

# # Extra steps beyond the longest possible halving chain before the skip-based
# # guards are armed.  log2(initial_scale) halvings is the worst case (scale down
# # to 1); the margin absorbs one re-overflow after the first growth doubling.
# _LOSS_SCALE_WARMUP_MARGIN_STEPS: int = 2


# def _resolve_initial_loss_scale(steps_per_epoch: int) -> float:
#     """
#     Initial dynamic loss scale, sized so calibration completes inside the first
#     epoch.

#     Worst case is ``log2(initial_scale)`` consecutive skipped steps (each
#     overflow halves the scale).  Budget at most half of one epoch for that
#     chain, then clamp into ``[2**8, 2**15]``::

#         steps_per_epoch=19  ->  2**9  = 512    (<= 9 warmup steps of 19)
#         steps_per_epoch=32  ->  2**15 = 32768  (Keras default; fits)

#     The caller keeps ``dynamic=True``, so the scale grows back up from whatever
#     level calibration settles at — this only changes where it STARTS.
#     """
#     _exp = int(max(int(steps_per_epoch), 1) // 2)
#     _exp = min(_MAX_INITIAL_LOSS_SCALE_EXP,
#                max(_MIN_INITIAL_LOSS_SCALE_EXP, _exp))
#     return float(2 ** _exp)


# def _loss_scale_growth_steps(steps_per_epoch: int) -> int:
#     """
#     Consecutive finite steps required before the dynamic scale doubles.

#     Keras defaults to 2000 — longer than an entire short run, which would pin
#     the scale at whatever low value calibration settled on and reintroduce the
#     fp16 underflow the scaling exists to prevent.  One epoch of clean steps
#     (bounded to ``[8, 2000]``) lets the scale climb back within the run while
#     still being long enough not to oscillate against a real overflow ceiling.
#     """
#     return int(min(2000, max(8, int(steps_per_epoch))))


# def _loss_scale_warmup_steps(initial_scale: Optional[float]) -> int:
#     """
#     Number of global steps during which skipped steps carry no diagnostic
#     weight on the MP path: the longest possible halving chain from
#     ``initial_scale`` down to 1, plus a small margin.
#     """
#     if not initial_scale:
#         return 0
#     try:
#         _halvings = int(math.ceil(math.log2(max(float(initial_scale), 1.0))))
#     except (ValueError, OverflowError):
#         _halvings = _MAX_INITIAL_LOSS_SCALE_EXP
#     return max(1, _halvings + _LOSS_SCALE_WARMUP_MARGIN_STEPS)


# def _is_loss_scale_optimizer(optimizer) -> bool:
#     """True when ``optimizer`` wraps its inner optimizer in dynamic loss scaling."""
#     return isinstance(optimizer, tf.keras.mixed_precision.LossScaleOptimizer)


# def _read_loss_scale(optimizer) -> Optional[float]:
#     """
#     Current dynamic loss scale, or ``None`` when the optimizer is not a
#     LossScaleOptimizer (fp32/CPU path) or has not been built yet.

#     Keras 3 keeps it in ``dynamic_scale``, a variable created in ``build()``;
#     Keras 2's ``tf.keras`` exposed the same value as ``loss_scale``.  Both are
#     probed so this reads correctly on either.  Nothing in this module read the
#     scale before, which is what made this failure mode undiagnosable: the logs
#     could not distinguish "calibrating" from "gradients are genuinely NaN".
#     """
#     if not _is_loss_scale_optimizer(optimizer):
#         return None
#     for _attr in ("dynamic_scale", "loss_scale"):
#         _v = getattr(optimizer, _attr, None)
#         if _v is None:
#             continue
#         try:
#             return float(_v.numpy()) if hasattr(_v, "numpy") else float(_v)
#         except Exception:
#             continue
#     return None


# def _loss_scale_report(optimizer, previous: Optional[float] = None) -> str:
#     """
#     ``"  loss_scale=512 (2^9, halving from 1024)"`` — the fragment appended to
#     every skip / abort message under mixed precision; empty string on the fp32
#     path so those messages are unchanged there.

#     The trend against ``previous`` is the diagnostic that separates the two
#     causes of a skipped step: a scale that HALVES on each skip is the
#     controller calibrating (benign, self-limiting); a scale PINNED while steps
#     keep being skipped means the gradients are non-finite at any representable
#     scale and the run is genuinely broken.
#     """
#     _scale = _read_loss_scale(optimizer)
#     if _scale is None:
#         return ""
#     _exp = ""
#     if _scale > 0:
#         try:
#             _exp = f", 2^{math.log2(_scale):.0f}"
#         except (ValueError, OverflowError):
#             _exp = ""
#     _trend = ""
#     if previous is not None and math.isfinite(previous) and previous > 0:
#         if _scale < previous:
#             _trend = f", halving from {previous:g}"
#         elif _scale > previous:
#             _trend = f", growing from {previous:g}"
#         else:
#             _trend = ", unchanged"
#     _parts = [p for p in (_exp.lstrip(", "), _trend.lstrip(", ")) if p]
#     return (
#         f"  loss_scale={_scale:g}"
#         + (f" ({', '.join(_parts)})" if _parts else "")
#     )

# # ── First-NaN origin diagnostics (opt-in) ───────────────────────────────────
# # The loss sanitizers destroy the evidence of WHERE a NaN originates: by the
# # time any host-visible scalar is inspected, every non-finite intermediate has
# # already been replaced with 0 / 0.5.  With ``YOLO_PRO_NAN_DEBUG=1`` the train
# # step additionally evaluates each candidate origin tensor UNSANITISED (a
# # second, gradient-free pass outside the tape) and returns a per-tensor
# # finiteness vector.  The first time a raw non-finite step is seen, that vector
# # — plus the global step and LR at which it happened — is latched into device
# # variables and logged, naming the tensor that actually went bad first.
# # Off by default: the probe roughly doubles the loss math per step.
# _NAN_DEBUG: bool = os.environ.get("YOLO_PRO_NAN_DEBUG", "0").strip().lower() in (
#     "1", "true", "yes", "on",
# )

# # ── Raw-gradient diagnostics ────────────────────────────────────────────────
# # Why this exists: a NaN can be born in the BACKWARD pass while every
# # host-visible forward signal reads clean.  That is not hypothetical — it is
# # what happened.  `tf.math.log(1 - p)` in the Varifocal loss evaluated log(0)
# # because its clamp `1.0 - 1e-9` rounds to exactly 1.0 in float32 (and the
# # fp16 head saturates sigmoid to exactly 1.0 at a logit of 7.623), so the Log
# # gradient was 1/0 = inf and the loss sanitiser's `tf.where` turned it into
# # 0 * inf = NaN.  Meanwhile the raw head outputs were finite, `total_loss` was
# # finite (the sanitiser masked the -inf), and the BN moving stats were finite.
# # Every signal the loop watched said "healthy" while no step could be applied.
# # The gradients themselves were the only place the truth was visible, and
# # nothing looked at them.
# #
# # These stats are measured on the gradients EXACTLY as `tape.gradient` returns
# # them — before `_sanitize_gradient`, before the optimizer's `global_clipnorm`,
# # before any unscaling, and before `apply_gradients`.  Under mixed precision
# # they are therefore the SCALED gradients, which is precisely the tensor the
# # LossScaleOptimizer's own overflow check sees.
# _GRAD_STAT_LEN: int = 8
# _GRAD_STAT_LABELS: Tuple[str, ...] = (
#     "n_grads",          # gradient tensors returned (excluding None)
#     "n_none",           # variables that received no gradient at all
#     "first_nan_idx",    # index into model.trainable_variables, -1 if none
#     "first_inf_idx",    # index into model.trainable_variables, -1 if none
#     "global_norm",      # tf.linalg.global_norm over the RAW grads (may be NaN)
#     "max_abs",          # max |g| over FINITE elements only
#     "min_finite_abs",   # min non-zero |g| over finite elements, -1 if none
#     "n_nonfinite",      # number of gradient tensors containing NaN or Inf
# )
# (
#     _GS_N, _GS_NONE, _GS_NAN_IDX, _GS_INF_IDX,
#     _GS_NORM, _GS_MAX, _GS_MIN, _GS_NBAD,
# ) = range(_GRAD_STAT_LEN)

# # Per-step host logging of the gradient stats.  The stats are ALWAYS computed
# # on-device (no host sync); this flag additionally forces a `.numpy()` readback
# # and a log line on EVERY step, which serialises the device.  Off by default:
# # the stats are otherwise reported at the existing readback cadence, plus the
# # first non-finite step is latched and reported in full whatever the cadence.
# _GRAD_DEBUG: bool = os.environ.get("YOLO_PRO_GRAD_DEBUG", "0").strip().lower() in (
#     "1", "true", "yes", "on",
# )

# # Kill switch for the measurement itself (the stats above are ON by default).
# # Cost: ~5 device-side reductions per gradient tensor per step — about 1 000
# # extra reduction ops on this model's 198 trainable tensors.  They are pure
# # device ops over memory that is already resident and XLA fuses them into the
# # existing graph, so the measured overhead is small, but this loop was
# # deliberately built to avoid per-step work and a way out belongs here.
# # Set YOLO_PRO_GRAD_DIAG=0 to skip the computation; the step then returns a
# # sentinel vector of -1 and every consumer degrades to "not measured".
# _GRAD_DIAG: bool = os.environ.get("YOLO_PRO_GRAD_DIAG", "1").strip().lower() not in (
#     "0", "false", "no", "off",
# )

# # Opt-in `tf.debugging.enable_check_numerics()`.  This is the tool that names
# # the exact graph op — it reported `Log` at yolo_pro/losses.py:205 for the
# # failure above.  It is far too slow for production (it inserts a
# # CheckNumericsV2 after every float op) and it raises rather than returning, so
# # it is opt-in and documented rather than wired into the abort path.
# _CHECK_NUMERICS: bool = os.environ.get(
#     "YOLO_PRO_CHECK_NUMERICS", "0"
# ).strip().lower() in ("1", "true", "yes", "on")

# # Loss scale at or below which recovery is impossible / the diagnosis is
# # already certain.
# #   • Exactly 0: `scale_loss` multiplies the loss by 0, so the scaled loss is
# #     0 and NO gradient information exists at all.  The controller can only
# #     halve from here; it can never grow off zero.  Unrecoverable by
# #     construction.
# #   • Below 2**-16: the gradients still overflow float16 even after the loss
# #     has been scaled DOWN by 65536x.  For that to be a true overflow the
# #     unscaled gradients would have to exceed 4.3e9.  It is not an overflow —
# #     it is a NaN, and no further halving will change it.
# # Aborting here instead of waiting for the scale to reach 0 saves the ~100
# # steps of guaranteed-useless training that the observed run spent decaying
# # through the subnormals.
# _MIN_VIABLE_LOSS_SCALE: float = 2.0 ** -16


# def _raw_gradient_stats(grads: List) -> tf.Tensor:
#     """
#     On-device summary of the RAW gradients, as a float32 vector laid out by
#     ``_GRAD_STAT_LABELS``.  Pure device ops — costs no host sync.

#     Called before every form of sanitisation, clipping and unscaling, so what
#     it reports is what the optimizer's overflow check actually sees.

#     ``IndexedSlices`` are summarised through their dense ``.values``.  Index
#     positions are into ``model.trainable_variables``, so the host can name the
#     offending tensor.  Extremes (``max_abs`` / ``min_finite_abs``) are taken
#     over finite elements only — a raw max would just read NaN and tell you
#     nothing about the magnitudes; ``global_norm`` is deliberately left raw so
#     a NaN there is itself a reported signal.
#     """
#     _idx: List[float] = []
#     _has_nan: List[tf.Tensor] = []
#     _has_inf: List[tf.Tensor] = []
#     _maxes:   List[tf.Tensor] = []
#     _mins:    List[tf.Tensor] = []
#     _flat:    List[tf.Tensor] = []
#     _n_none = 0
#     _inf32 = tf.constant(float("inf"), dtype=tf.float32)
#     for _i, _g in enumerate(grads):
#         if _g is None:
#             _n_none += 1
#             continue
#         _t = _g.values if isinstance(_g, tf.IndexedSlices) else _g
#         _t = tf.cast(_t, tf.float32)
#         _a = tf.abs(_t)
#         _fin = tf.math.is_finite(_t)
#         _idx.append(float(_i))
#         _has_nan.append(tf.reduce_any(tf.math.is_nan(_t)))
#         _has_inf.append(tf.reduce_any(tf.math.is_inf(_t)))
#         _maxes.append(tf.reduce_max(tf.where(_fin, _a, tf.zeros_like(_a))))
#         _mins.append(tf.reduce_min(tf.where(
#             tf.logical_and(_fin, _a > 0.0), _a, tf.fill(tf.shape(_a), _inf32)
#         )))
#         _flat.append(_t)

#     if not _flat:
#         return tf.stack([
#             tf.constant(0.0), tf.constant(float(_n_none)),
#             tf.constant(-1.0), tf.constant(-1.0),
#             tf.constant(float("nan")), tf.constant(-1.0), tf.constant(-1.0),
#             tf.constant(0.0),
#         ])

#     _idx_t   = tf.constant(_idx, dtype=tf.float32)
#     _nan_v   = tf.stack(_has_nan)
#     _inf_v   = tf.stack(_has_inf)
#     _sentinel = tf.constant(1e12, dtype=tf.float32)

#     def _first(_mask):
#         _m = tf.reduce_min(tf.where(_mask, _idx_t, tf.fill(tf.shape(_idx_t), _sentinel)))
#         return tf.where(_m >= _sentinel, tf.constant(-1.0), _m)

#     _min_abs = tf.reduce_min(tf.stack(_mins))
#     return tf.stack([
#         tf.constant(float(len(_flat))),
#         tf.constant(float(_n_none)),
#         _first(_nan_v),
#         _first(_inf_v),
#         tf.linalg.global_norm(_flat),
#         tf.reduce_max(tf.stack(_maxes)),
#         tf.where(tf.math.is_finite(_min_abs), _min_abs, tf.constant(-1.0)),
#         tf.reduce_sum(tf.cast(tf.logical_or(_nan_v, _inf_v), tf.float32)),
#     ])


# def _format_grad_stats(stats, var_names: Optional[List[str]] = None) -> str:
#     """
#     Render a ``_raw_gradient_stats`` vector as one log line.

#     ``stats`` is a numpy array / sequence of ``_GRAD_STAT_LEN`` floats.
#     ``var_names`` maps the first-NaN / first-Inf indices back to variable
#     names; without it the raw indices are printed.
#     """
#     def _name(_i: float) -> str:
#         _i = int(_i)
#         if _i < 0:
#             return "none"
#         if var_names is not None and 0 <= _i < len(var_names):
#             return f"[{_i}] {var_names[_i]}"
#         return f"[{_i}]"

#     if int(stats[_GS_N]) < 0:
#         return "grad_stats=not measured (YOLO_PRO_GRAD_DIAG=0)"
#     _norm = float(stats[_GS_NORM])
#     _min  = float(stats[_GS_MIN])
#     return (
#         f"grad_tensors={int(stats[_GS_N])} none={int(stats[_GS_NONE])} "
#         f"nonfinite_tensors={int(stats[_GS_NBAD])} "
#         f"first_nan={_name(stats[_GS_NAN_IDX])} "
#         f"first_inf={_name(stats[_GS_INF_IDX])} "
#         f"global_norm={_norm:.6g} "
#         f"max_abs={float(stats[_GS_MAX]):.6g} "
#         f"min_finite_abs={('n/a' if _min < 0 else f'{_min:.6g}')}"
#     )


# def _optimizer_iteration(optimizer) -> int:
#     """Optimizer step counter, or -1 when it cannot be read."""
#     for _attr in ("iterations", "_iterations"):
#         _v = getattr(optimizer, _attr, None)
#         if _v is None:
#             continue
#         try:
#             return int(_v.numpy()) if hasattr(_v, "numpy") else int(_v)
#         except Exception:
#             continue
#     return -1

# # Probe order inside the diagnostic vector returned by `_train_step_impl`.
# # Ordered along the forward path so the FIRST failing label is the origin:
# #   cls_pred / reg_pred  → raw head outputs
# #   dfl_log_softmax      → the DFL bin distribution (softmax over reg_pred)
# #   pred_boxes           → unsanitised DFL→ltrb→xyxy decode
# #   vfl / ciou / dfl     → the three loss terms computed with sanitize=False
# _NAN_DIAG_TENSORS: Tuple[str, ...] = (
#     "cls_pred", "reg_pred", "dfl_log_softmax", "pred_boxes",
#     "vfl", "ciou", "dfl",
# )
# _NAN_DIAG_LABELS: Tuple[str, ...] = tuple(
#     f"{_scale}/{_tensor}"
#     for _scale in ("p3", "p4", "p5")
#     for _tensor in _NAN_DIAG_TENSORS
# )
# _NAN_DIAG_LEN: int = len(_NAN_DIAG_LABELS)
...
# # ── Background false-positive gate on best-checkpoint ranking ────────────────
# # `evaluate_yolo_pro_detection` reports `background_fp_rate`: post-NMS
# # detections per GT-free ("background") test image, measured at the runtime
# # confidence threshold the deployed model actually applies.  mAP is near-blind
# # to this by construction — an image with no GT contributes no AP term — so a
# # checkpoint can win on mAP50/precision while firing several phantom boxes on
# # every empty CCTV frame.  That is the exact failure this gate exists to stop.
# #
# # The gate is ADDITIVE, not a replacement: it is the leading (most significant)
# # tier of `_export_candidate_rank_key`, so among candidates that share a tier
# # the existing mAP50 → precision → tp_fp_gap → preds-per-image → epoch order is
# # untouched.  Two deliberate properties:
# #   • Datasets with NO background images report `background_fp_rate is None`,
# #     which passes.  Every candidate lands in the same tier and the ranking is
# #     bit-for-bit what it was before this gate existed.
# #   • The gate DEMOTES rather than hard-excludes.  A hard exclusion could empty
# #     the finalist pool and fail an otherwise-successful run with
# #     `_NO_SAFE_CKPT_ERROR`; demotion means that if every candidate is over the
# #     ceiling the least-bad one still ships, and the log says why.
# #
# # 0.30 detections per background image: on a healthy detector a background
# # frame should fire ~never at deployment confidence, so ~1 phantom box every
# # 3+ empty frames is already the "this will spam the operator" regime, while
# # still leaving headroom for a single hard frame in a small test split.
# _CKPT_BG_FP_RATE_MAX: float = 0.30
...
#       0. background-FP gate  (1 = passes, 0 = over the ceiling)
...
#     Tier 0 is the background false-positive gate (see
#     ``_CKPT_BG_FP_RATE_MAX``): a candidate whose measured
#     ``background_fp_rate`` exceeds the ceiling sorts below every candidate
#     that stays under it, regardless of mAP.  It is a gate, not a metric — the
#     magnitude of the overshoot is deliberately NOT compared, so tiers 1-5 stay
#     the only thing that orders candidates on the same side of it.

#     The gate PASSES (value 1) whenever the rate is absent or non-finite, which
#     is the case for every entry on a dataset with no background images and for
#     every entry written by an older worker run.  Ranking on such datasets is
#     therefore bit-for-bit identical to the pre-gate behaviour: a constant
#     leading element cannot change the outcome of a tuple comparison.
...
#       • background_fp_rate missing            → gate passes (see above)
...
#     # Background-FP gate.  nan is mapped to the pass value, not to a
#     # comparison against the ceiling, so an absent/garbage rate can never
#     # demote a candidate on absence of signal (same no-op-on-missing rule the
#     # detector-quality gates follow).
#     bg_fp = _pick(entry, "full_background_fp_rate", "background_fp_rate", float("nan"))
#     bg_ok = 0 if (math.isfinite(bg_fp) and bg_fp > _CKPT_BG_FP_RATE_MAX) else 1
#     # Negate runtime preds so larger tuple value = lower (better) preds.
#     return (bg_ok, map50, precision, tp_fp_gap, -rt_ppi, index)
...
#         reg_max    : DFL bins
#         sanitize   : when True (default) non-finite logits and output boxes are
#                      replaced with 0.  Pass False ONLY from the first-NaN
#                      diagnostic probe, which needs the unmasked decode to tell
#                      whether the non-finiteness entered at the head output or
#                      was manufactured by the decode itself.
...
# ) -> tf.Tensor:
#     """
#     ``apply_gradients`` wrapped so it can be used as a ``tf.cond`` branch.

#     ``tf.cond`` requires both branches to return the same structure, so this
#     returns a scalar bool the caller can hang a control dependency on.  Keeping
#     it a module-level function (rather than an inline lambda) makes the traced
#     graph readable and keeps the branch free of Python closures over tensors.

#     The optimizer's slot variables MUST already be built (see
#     ``optimizer.build(...)`` at setup) — a ``tf.cond`` branch that creates a
#     variable is not traceable under ``tf.function``.
#     """
#     optimizer.apply_gradients(zip(grads, variables))
#     return tf.constant(True)


# def _sanitize_gradient(g):
#     """
#     Replace non-finite gradient elements with 0, preserving the container type.

#     Defence in depth for the ``global_clipnorm=10.0`` amplifier: the clip is a
#     GLOBAL reduction, so one NaN element makes ``global_norm`` NaN, and the
#     resulting NaN scale factor multiplies into EVERY variable's gradient.  One
#     bad element in one tensor would otherwise poison the whole model in a single
#     step.  On a finite step this is a bit-exact no-op, so the clip's behaviour on
#     healthy steps — including its rescaling of genuinely large gradients — is
#     completely unchanged; after sanitisation the global norm is guaranteed
#     finite, which is exactly the condition under which the clip is meaningful.
#     """
#     if g is None:
#         return None
...
#             tf.where(
#                 tf.math.is_finite(g.values), g.values, tf.zeros_like(g.values)
#             ),
#             g.indices,
#             g.dense_shape,
#         )
...
# # with `jit_compile` gated on this worker process's configured device
# # (`worker_uses_gpu()`) — see `_train_step = tf.function(...)` near the
# # model-build site. Keep this body pure/traceable.
...
#     Returns:
#         total_loss  : scalar float32, POST-sanitiser (finite by construction).
#         o2m_total   : scalar float32.
#         o2o_total   : scalar float32.
#         step_finite : scalar bool — gradients AND total_loss finite.  When
#                       False the weight update is a true no-op (see the apply
#                       gate below) and the host suppresses EMA + accumulators.
#         raw_finite  : scalar bool — every RAW head output (``cls_pred`` /
#                       ``reg_pred``, all three scales) was finite BEFORE any
#                       sanitiser ran.  This is the only honest NaN signal the
#                       step produces: ``total_loss`` cannot report one because
#                       every loss term ends in ``tf.where(is_finite(x), x, 0)``.
#                       The host accumulates ``not raw_finite`` into
#                       ``_acc_raw_nonfinite_count`` and aborts on it.
#         diag        : float32 vector of length ``_NAN_DIAG_LEN``, one entry per
#                       ``_NAN_DIAG_LABELS`` probe: 1.0 finite, 0.0 non-finite,
#                       or all ``-1.0`` when ``YOLO_PRO_NAN_DEBUG`` is off.
#     """
#     from app.ml.yolo_pro.losses import yolo_pro_loss

#     # Raw (pre-sanitisation) finiteness flags, one pair per scale.  Collected
#     # inside the tape but never differentiated — `is_finite` returns bool, so
#     # these ops contribute nothing to the backward pass.
#     _raw_finite_flags: List[tf.Tensor] = []
#     # Per-scale tensors stashed for the opt-in first-NaN probe, which runs
#     # AFTER the tape closes so the extra loss math costs no tape memory.
#     _dbg_scales: List[tuple] = []
...
#             # ── UNSANITISED finiteness probe ─────────────────────────────────
#             # Must be read HERE — `_decode_reg_scale` and every loss term below
#             # replace non-finite values, after which the failure is invisible.
#             # The fp16→fp32 cast preserves NaN/Inf, so a float16 overflow in the
#             # head is caught exactly like an fp32 one.
#             _raw_finite_flags.append(tf.reduce_all(tf.math.is_finite(cls_pred)))
#             _raw_finite_flags.append(tf.reduce_all(tf.math.is_finite(reg_pred)))
#             if _NAN_DEBUG:
#                 _dbg_scales.append(
#                     (cls_pred, reg_pred, cls_t, box_t, ltrb_t, fg, anch, stride_t)
#                 )
...
#     # ── Raw-gradient diagnostics ────────────────────────────────────────────
#     # Measured HERE and nowhere else: this is the last point at which `grads`
#     # is untouched.  Below, the fp32 branch runs `_sanitize_gradient` and both
#     # branches hand the tensors to an optimizer that clips them.  Under mixed
#     # precision these are the SCALED gradients — the same tensors the
#     # LossScaleOptimizer's internal `check_finite` inspects — so a disagreement
#     # between `step_finite` and the optimizer's skip decision is impossible by
#     # construction, and the reported magnitudes divide by the loss scale to
#     # give the true ones.  See `_GRAD_STAT_LABELS`.
#     grad_stats = (
#         _raw_gradient_stats(grads) if _GRAD_DIAG
#         else tf.fill([_GRAD_STAT_LEN], tf.constant(-1.0, dtype=tf.float32))
#     )

#     # Finiteness flag drives the weight-update gate below AND EMA + accumulator
#     # suppression on the host side (a bad step must not update the EMA or
#     # pollute the epoch mean).
#     #
#     # WHICH tensors is this computed from?  `grads` straight off the tape:
#     #   • RAW        — no `_sanitize_gradient` has run (that is fp32-branch
#     #                  only, and happens after this).
#     #   • UNCLIPPED  — `global_clipnorm=10.0` lives on the INNER AdamW and runs
#     #                  inside `apply_gradients`, after the overflow check.
#     #   • SCALED     — under mixed precision the tape differentiated
#     #                  `optimizer.scale_loss(total_loss)`, so these are the
#     #                  scaled gradients.  That is the same tensor
#     #                  `LossScaleOptimizer.check_finite` reduces over, so this
#     #                  flag and the optimizer's skip decision cannot disagree.
#     # Scaling by a finite non-zero factor preserves finiteness, so the flag is
#     # identical to one taken on the unscaled gradients — EXCEPT for a genuine
#     # fp16 overflow, which is exactly what the scaled check is supposed to
#     # catch.  A NaN, by contrast, survives any scaling: that is why a scale
#     # that halves forever proves the problem is not an overflow.
#     _finite_flags = [
...
#     # Honest NaN signal: the head outputs before anything masked them.
#     if _raw_finite_flags:
#         raw_finite = tf.reduce_all(tf.stack(_raw_finite_flags))
#     else:
#         raw_finite = tf.constant(True)

#     # ── Weight-update gate ──────────────────────────────────────────────────
#     _tvars = model.trainable_variables
#     if isinstance(optimizer, tf.keras.mixed_precision.LossScaleOptimizer):
#         # MP/GPU path: apply UNCONDITIONALLY and with the raw gradients.  The
#         # LossScaleOptimizer's own internal `tf.cond` already makes a non-finite
#         # step a true no-op for the weights — and it must SEE the overflow to
#         # halve the dynamic loss scale so the next step can succeed.  Skipping
#         # or sanitising here would hide the overflow from the scale controller
#         # and pin the scale too high forever, turning a transient overflow into
#         # a permanent one.  It unscales → clips (inner global_clipnorm=10.0) →
#         # applies, or skips + lowers the scale.
#         optimizer.apply_gradients(zip(grads, _tvars))
#     else:
#         # fp32/CPU path: a plain AdamW has NO skip logic, so before this gate a
#         # NaN gradient was applied to the weights — `step_finite` only
#         # suppressed EMA and the host accumulators, and the model was already
#         # destroyed by the time the next epoch's BN probe noticed.  Zeroing the
#         # gradients is NOT sufficient on its own: Keras 3's
#         # `_backend_apply_gradients` applies decoupled weight decay and updates
#         # the Adam moments regardless of the gradient value.  So the entire
#         # apply is wrapped in `tf.cond` and a bad step becomes a genuine no-op
#         # for every weight.  Gradients are sanitised as defence in depth (see
#         # `_sanitize_gradient`) in case `step_finite` and the gradients ever
#         # disagree.  Traceable under `tf.function` because the optimizer's slot
#         # variables are pre-built at setup, so no branch creates a variable.
#         _safe_grads = [_sanitize_gradient(g) for g in grads]
#         _applied = tf.cond(
#             step_finite,
#             lambda: _apply_and_flag(optimizer, _safe_grads, _tvars),
#             lambda: tf.constant(False),
#         )
#         # Anchor the cond to a returned value so it can never be pruned.
#         with tf.control_dependencies([_applied]):
#             total_loss = tf.identity(total_loss)

#     # ── First-NaN origin probe (opt-in, outside the tape) ───────────────────
#     # Recomputes each candidate origin tensor with every sanitiser disabled and
#     # reports per-tensor finiteness, so the host can name WHICH tensor went
#     # non-finite first instead of only that something did.  Ordered along the
#     # forward path (see `_NAN_DIAG_LABELS`).
#     if _NAN_DEBUG and _dbg_scales:
#         _diag_flags: List[tf.Tensor] = []
#         for (_d_cls, _d_reg, _d_clst, _d_boxt, _d_ltrbt, _d_fg,
#              _d_anch, _d_stride) in _dbg_scales:
#             _d_cls = tf.stop_gradient(_d_cls)
#             _d_reg = tf.stop_gradient(_d_reg)
#             _d_boxes = _decode_reg_scale(
#                 _d_reg, _d_anch,
#                 stride=tf.cast(_d_stride, tf.float32),
#                 H_img=H_img, W_img=W_img, reg_max=reg_max,
#                 sanitize=False,
#             )
#             _d_logits = tf.reshape(
#                 _d_reg, (tf.shape(_d_reg)[0], -1, 4, reg_max)
#             )
#             _d_lsm = tf.nn.log_softmax(_d_logits, axis=-1)
#             _d_losses = yolo_pro_loss(
#                 cls_pred=_d_cls,     reg_pred=_d_reg,
#                 cls_targets=_d_clst, box_targets=_d_boxt,
#                 ltrb_targets=_d_ltrbt, fg_mask=_d_fg,
#                 pred_boxes=_d_boxes, reg_max=reg_max,
#                 w_cls=w_cls, w_box=w_box, w_dfl=w_dfl,
#                 sanitize=False,
#             )
#             _diag_flags.extend([
#                 tf.reduce_all(tf.math.is_finite(_d_cls)),
#                 tf.reduce_all(tf.math.is_finite(_d_reg)),
#                 tf.reduce_all(tf.math.is_finite(_d_lsm)),
#                 tf.reduce_all(tf.math.is_finite(_d_boxes)),
#                 tf.math.is_finite(_d_losses["loss_cls"]),
#                 tf.math.is_finite(_d_losses["loss_box"]),
#                 tf.math.is_finite(_d_losses["loss_dfl"]),
#             ])
#         diag = tf.cast(tf.stack(_diag_flags), tf.float32)
#     else:
#         # Sentinel: -1 means "probe not enabled", distinguishable from 0
#         # ("this tensor was non-finite").
#         diag = tf.fill([_NAN_DIAG_LEN], tf.constant(-1.0, dtype=tf.float32))

#     return (
#         total_loss, o2m_total, o2o_total, step_finite, raw_finite, diag,
#         grad_stats,
#     )
...
# def _use_mosaic_for_sample(
#     boxes: Optional[List[dict]],
#     mosaic_epoch_gate: bool,
#     mosaic_prob: float,
# ) -> bool:
#     """
#     Decide whether ONE training sample is mosaicked, given the epoch gate and
#     the configured per-sample probability.

#     Three gates, evaluated in this order:
#       1. epoch gate  — ``train_cfg.use_mosaic(epoch)``; False in the
#                        close-mosaic refinement phase.
#       2. background  — a sample with zero GT boxes is NEVER the anchor of a
#                        mosaic.
#       3. probability — ``np.random.random() < mosaic_prob``.

#     Gate 2 is the false-positive fix.  A 2x2 mosaic replaces three quarters of
#     the anchor image with crops of other images, so mosaicking a background
#     frame destroys the one training signal that says "this entire scene
#     contains nothing": what reaches the head is a mostly-foreground composite
#     carrying the boxes of its three donors.  On a CCTV dataset — where the
#     negatives ARE the hard cases (empty corridors, moving shadows, parked
#     cars) — that silently deletes most of the negative supervision the user
#     added, and the model keeps firing on empty frames.  Keeping the negative
#     whole preserves its full-frame context as a single all-negative anchor
#     grid, which is what suppresses those detections.

#     Positives are untouched: they take exactly the previous path.

#     RNG parity: the probability draw is the LAST gate and is skipped by
#     short-circuit whenever an earlier gate fails.  On a dataset with no
#     background images gate 2 never fires, so the np.random consumption
#     sequence is byte-identical to the pre-change inline expression
#     (``mosaic_epoch_gate and np.random.random() < mosaic_prob``) and
#     augmentation on ordinary detection datasets is bit-for-bit unregressed.

#     Args:
#         boxes             : the sample's GT box list (None / [] == background)
#         mosaic_epoch_gate : epoch-level mosaic switch for this epoch
#         mosaic_prob       : per-sample mosaic probability from TrainConfig

#     Returns:
#         True when this sample should be composed into a mosaic.
#     """
#     if not mosaic_epoch_gate:
#         return False
#     if not (boxes or []):
#         return False
#     return bool(np.random.random() < mosaic_prob)
...
#     # Enable the mixed_float16 policy ONLY when THIS worker process will run on
#     # a GPU — float16 is a slowdown on CPU and offers no benefit.  The probe is
#     # `worker_uses_gpu()` (the device frozen by init_worker_device at worker
#     # boot), NOT `tf.config.list_physical_devices("GPU")`: that lists host
#     # hardware and ignores the `set_visible_devices([], "GPU")` that a CPU
#     # worker performs, so on a GPU host a WORKER_DEVICE=cpu job would build the
#     # whole model in float16 and run it on CPU.
#     # The policy is set *before* the model is built so every layer captures
#     # float16 compute / float32 variables, then restored to the previous (fp32)
#     # global policy immediately after: already-constructed layers keep their
#     # per-layer policy, while any model rebuilt later (e.g. the TFLite
#     # export/decode graph) stays fp32.
#     if _CHECK_NUMERICS:
#         # Names the exact graph op that first emits a NaN/Inf — including ops
#         # in the BACKWARD pass, which is where the failure that motivated this
#         # lived (`Log` in yolo_pro/losses.py, from log(1 - p) with p == 1.0).
#         # Raises rather than returning, and inserts a CheckNumericsV2 after
#         # every float op, so this is a debugging run, not a training run.
#         tf.debugging.enable_check_numerics()
#         logger.warning(
#             f"[{job_id}] YOLO_PRO_CHECK_NUMERICS=1 — check_numerics is ON. "
#             f"Training will ABORT at the first non-finite tensor and name the "
#             f"op that produced it. Expect a large slowdown."
#         )

#     _prev_mp_policy      = tf.keras.mixed_precision.global_policy()
#     _use_mixed_precision = worker_uses_gpu()
...
#     #   2. `_train_step` was statically `@tf.function(jit_compile=True)`, so a
#     #      CPU worker paid a large one-time XLA compile for zero runtime gain
#     #      (and float32, since MP is GPU-gated). Gate XLA on the SAME device
#     #      decision used for mixed precision so XLA and MP flip together: both
#     #      on GPU only.
#     # Deliberately reuses `_use_mixed_precision` rather than re-probing — one
#     # source of truth for "this process is on a GPU" (see its definition above,
#     # and the LossScaleOptimizer wrap further down, which reads the same flag).
#     _use_xla = _use_mixed_precision
...
#         f"jit_compile(XLA)={'ON' if _use_xla else 'OFF (CPU worker)'}  "
...
#         f"(over {dataset_stats['labeled_images']} labeled images)  "
#         f"negative_image_fraction={dataset_stats['negative_image_fraction']:.3f}  "
...
#     )
#     # Negative (background-image) ratio cap actually in force for this run.
#     # The cap is APPLIED upstream in the dataset loader
#     # (`_cap_detection_negatives`); this line only reports the resolved value
#     # so the training log is self-contained -- a reader diagnosing background
#     # false positives can see the configured ceiling next to the achieved
#     # `negative_image_fraction` above without cross-referencing the loader log.
#     # Resolution is delegated to the single source of truth in training_worker
#     # (extra_params > impulse ml_blocks params > default), imported locally
#     # because training_worker imports this module at module scope.
#     try:
#         from app.workers.training_worker import (
#             _resolve_negative_ratio_cap as _resolve_neg_cap,
#             NEGATIVE_RATIO_CAP_DEFAULT as _NEG_CAP_DEFAULT,
#             NEGATIVE_RATIO_CAP_MAX as _NEG_CAP_MAX,
#         )
#         _neg_cap_in_force = _resolve_neg_cap(impulse, job.extra_params or {})
#         _neg_cap_src = (
#             "default" if _neg_cap_in_force == _NEG_CAP_DEFAULT else "configured"
#         )
#         _neg_cap_line = (
#             f"Negative-image cap: {_neg_cap_in_force:.0%} of the training set "
#             f"({_neg_cap_src}; max {_NEG_CAP_MAX:.0%})  "
#             f"achieved negative_image_fraction="
#             f"{dataset_stats['negative_image_fraction']:.3f}"
#         )
#         logger.info(f"[{job_id}] {_neg_cap_line}")
#     except Exception as _neg_cap_exc:
#         # Reporting-only: never let a log line abort a training run.
#         _neg_cap_line = None
#         logger.debug(
#             f"[{job_id}] negative-image cap could not be resolved for logging: "
#             f"{_neg_cap_exc}"
#         )
#     logger.info(
...
#     # Dynamic-scale calibration budget (MP path only).  `initial_scale` is
#     # derived from steps_per_epoch instead of the Keras default 2**15, which
#     # needs up to 15 consecutive skipped steps to calibrate — unreachable
#     # inside a 19-step epoch, so the run consumed its whole first epoch on
#     # warmup and the skip-ratio guard aborted it as "not training".
#     _initial_loss_scale: Optional[float] = None
#     _mp_warmup_steps: int = 0
#     if _use_mixed_precision:
#         _initial_loss_scale = _resolve_initial_loss_scale(steps_per_epoch)
#         _mp_warmup_steps    = _loss_scale_warmup_steps(_initial_loss_scale)
#         optimizer = tf.keras.mixed_precision.LossScaleOptimizer(
#             optimizer,
#             initial_scale=_initial_loss_scale,
#             # Grow back within the run: the Keras default of 2000 finite steps
#             # outlives most runs here, pinning the scale at its calibrated
#             # floor for the entire training.
#             dynamic_growth_steps=_loss_scale_growth_steps(steps_per_epoch),
#             # Keep the scale dynamic so it can still climb when gradients
#             # shrink later in training.  (Keras 3 is always dynamic and only
#             # accepts this kwarg as True; Keras 2 needs it explicitly.)
#             dynamic=True,
#         )
#         logger.info(
#             f"[{job_id}] Loss scaling: initial_scale={_initial_loss_scale:g} "
#             f"(2^{math.log2(_initial_loss_scale):.0f}, Keras default 2^15) "
#             f"dynamic_growth_steps="
#             f"{_loss_scale_growth_steps(steps_per_epoch)} "
#             f"worst-case calibration={_mp_warmup_steps} step(s) vs "
#             f"steps_per_epoch={steps_per_epoch}. Skipped steps inside that "
#             f"window are loss-scale calibration, not model corruption, and do "
#             f"not arm the skip-ratio abort; beyond it, a skipped step still "
#             f"only aborts when a raw non-finite head output, a non-finite "
#             f"total_loss, or a poisoned BN moving stat accompanies it."
#         )
...
#     # Build the optimizer's slot variables (Adam m/v, the iteration counter, and
#     # the LossScaleOptimizer's dynamic-scale pair) HERE, outside the traced
#     # training step.  Required — not merely an optimisation — since the fp32
#     # path gates `apply_gradients` behind a `tf.cond` so a non-finite step is a
#     # true weight no-op: Keras 3 builds slots lazily on the first
#     # `apply_gradients`, and a `tf.cond` branch that creates variables is not
#     # traceable under `tf.function`.
...
#     )
#     if _neg_cap_line:
#         _yolo_log_lines.append(_neg_cap_line)
#     _yolo_log_lines.append(
...
#     # Raw (pre-sanitisation) non-finite step counter — the abort signal.
#     # `_acc_loss_nan_count` counts steps whose POST-sanitiser `total_loss` was
#     # non-finite, which is structurally impossible (every loss term ends in a
#     # `tf.where(is_finite(x), x, 0)`), so it is retained for reporting only and
#     # is expected to read 0 even on a catastrophically poisoned epoch.
#     _acc_raw_nonfinite_count = tf.Variable(0.0, trainable=False, dtype=tf.float32)
#     # Last observed dynamic loss scale (MP path; None on fp32/CPU).  Carried
#     # across readbacks AND epochs so every log line can report the direction of
#     # travel — halving means the controller is calibrating, pinned means the
#     # gradients are non-finite at any scale.  Read here (post-`optimizer.build`)
#     # so the very first readback has a baseline to compare against.
#     # ── Raw-gradient diagnostics state ──────────────────────────────────────
#     # `_grad_var_names` maps the first-NaN / first-Inf INDEX reported by
#     # `_raw_gradient_stats` back to a variable name.  Captured once: the index
#     # is a position in `model.trainable_variables`, whose order is fixed for
#     # the life of the model.
#     _grad_var_names: List[str] = [
#         getattr(_v, "path", None) or getattr(_v, "name", f"var_{_i}")
#         for _i, _v in enumerate(model.trainable_variables)
#     ]
#     # Most recent step's stats, and a latch holding the FIRST step whose raw
#     # gradients contained a NaN or Inf.  Both are device-resident and written
#     # with pure device ops, so per-step measurement costs no host sync; they
#     # are read back at the usual cadence (or every step under
#     # YOLO_PRO_GRAD_DEBUG=1).
#     _grad_stats_last = tf.Variable(
#         tf.zeros([_GRAD_STAT_LEN], dtype=tf.float32),
#         trainable=False, dtype=tf.float32,
#     )
#     _grad_stats_first_bad = tf.Variable(
#         tf.fill([_GRAD_STAT_LEN], tf.constant(-1.0, dtype=tf.float32)),
#         trainable=False, dtype=tf.float32,
#     )
#     _grad_bad_step     = tf.Variable(-1.0, trainable=False, dtype=tf.float32)
#     _grad_bad_captured = tf.Variable(0.0,  trainable=False, dtype=tf.float32)
#     _grad_bad_logged: bool = False
#     _prev_loss_scale: Optional[float] = _read_loss_scale(optimizer)
#     # Global step at which the current epoch started — lets the epoch-end guard
#     # work out how many of its steps fell inside the loss-scale warmup window.
#     _epoch_first_global_step: int = 0
#     # Readback cadence.  A fixed 20 never fired on short epochs — this run had
#     # steps_per_epoch=16, so `_epoch_step % 20 == 0` was never true and the
#     # in-epoch abort check simply did not execute.  Scale with the epoch so the
#     # check runs ~4× per epoch on any dataset, while never syncing more often
#     # than every 20 steps on large ones.
#     _LOSS_READBACK_EVERY = min(20, max(1, steps_per_epoch // 4))
#     # ── First-NaN latch (device-resident; assigned with pure device ops) ─────
#     # Written only on the FIRST raw non-finite step, so the evidence survives
#     # the subsequent steps that would overwrite it.
#     _dbg_nan_vec = tf.Variable(
#         tf.fill([_NAN_DIAG_LEN], tf.constant(-1.0, dtype=tf.float32)),
#         trainable=False, dtype=tf.float32,
#     )
#     _dbg_nan_step     = tf.Variable(-1.0, trainable=False, dtype=tf.float32)
#     _dbg_nan_lr       = tf.Variable(0.0,  trainable=False, dtype=tf.float32)
#     _dbg_nan_captured = tf.Variable(0.0,  trainable=False, dtype=tf.float32)
#     _first_nan_logged: bool = False

#     # ── BN moving-stat finiteness probe ─────────────────────────────────────
#     # BN moving stats are updated inside the `training=True` forward pass —
#     # BEFORE gradients exist — so the per-step gradient gate cannot protect
#     # them.  This used to run only at epoch start, which meant poisoning at
#     # step 4 of 16 wasted the remaining 12 steps, a full validation pass, and
#     # produced a checkpoint from an already-dead model.  It now also runs at
#     # the in-epoch readback cadence.
#     _bn_vars = [
#         _v for _v in model.variables
#         if "moving_mean" in _v.name or "moving_variance" in _v.name
#     ]

#     def _bn_finite_flag() -> tf.Tensor:
#         """
#         Scalar bool tensor: are ALL BN moving stats finite?

#         Reduces every BN moving-stat variable to a scalar ON-DEVICE and stacks
#         them into one reduce, so callers can fold it into a single host sync
#         instead of paying one D2H stall per BN variable.
#         """
#         if not _bn_vars:
#             return tf.constant(True)
#         return tf.reduce_all(tf.stack([
#             tf.reduce_all(tf.math.is_finite(_v)) for _v in _bn_vars
#         ]))

#     def _bn_poisoned_variable_name() -> str:
#         """
#         Name the first BN variable with non-finite moving stats.

#         Cold path only — runs after `_bn_finite_flag()` has already reported a
#         failure, so the per-variable host sync it costs is irrelevant.
#         """
#         _flags = tf.stack([
#             tf.reduce_all(tf.math.is_finite(_v)) for _v in _bn_vars
#         ]).numpy()
#         return next(
#             (_v.name for _v, _ok in zip(_bn_vars, _flags) if not bool(_ok)),
#             "<unknown>",
#         )

#     def _first_nan_diagnostic() -> str:
#         """
#         Human-readable report of where the first raw non-finite value appeared.

#         Reads the latch written on the first raw non-finite step: the per-tensor
#         probe vector (when `YOLO_PRO_NAN_DEBUG=1`), plus the global step and LR
#         at which it happened.  Cold path — only called on the way to an abort.
#         """
#         if float(_dbg_nan_captured.numpy()) < 0.5:
#             return (
#                 "no raw non-finite head output was recorded this run — the "
#                 "non-finiteness entered through the BACKWARD pass (finite "
#                 "forward, non-finite gradients), so inspect the loss "
#                 "derivatives rather than the head outputs"
#             )
#         _vec  = _dbg_nan_vec.numpy()
#         _step = int(_dbg_nan_step.numpy())
#         _lr   = float(_dbg_nan_lr.numpy())
#         if _vec.size and float(_vec[0]) < 0.0:
#             _detail = (
#                 "per-tensor origin NOT captured — re-run with "
#                 "YOLO_PRO_NAN_DEBUG=1 to have the train step evaluate each "
#                 "candidate tensor unsanitised and name the true origin"
#             )
#         else:
#             _bad = [
#                 _name for _name, _ok in zip(_NAN_DIAG_LABELS, _vec)
#                 if float(_ok) < 0.5
#             ]
#             _detail = (
#                 "non-finite tensors (forward order): " + ", ".join(_bad)
#                 if _bad else
#                 "every probed tensor was finite — the non-finiteness entered "
#                 "between probes (inspect the backward pass)"
#             )
#         return (
#             f"first raw non-finite step: global_step={_step}  "
#             f"lr={_lr:.6g}  |  {_detail}"
#         )
...
#             # Mosaic decision, including the "backgrounds are never
#             # mosaicked" gate — see `_use_mosaic_for_sample` for why, and for
#             # the RNG-parity argument that keeps ordinary detection datasets
#             # bit-for-bit unchanged.  Backgrounds still receive hflip + HSV
#             # via the else-branch below; only the mosaic composite is skipped.
#             # Backgrounds remain eligible as mosaic *donors* (the `extra` draw
#             # below is unrestricted): an empty quadrant inside a positive
#             # mosaic is useful hard-negative context, not the case this gate
#             # is about.
#             if _use_mosaic_for_sample(
#                 boxes_train[i], mosaic_epoch_gate, train_cfg.mosaic_prob
#             ):
...
#         nonlocal _raw_nonfinite_logged, _first_nan_logged
#         nonlocal _epoch_positives_assigned, _prev_loss_scale
#         nonlocal _grad_bad_logged
...
#             # Recorded for the checkpoint sanity gate: a val_cls_pos of exactly
#             # 0.0 is only damning if TAL actually assigned positives to train
#             # against this epoch.
#             if (_pos_p3 + _pos_p4 + _pos_p5) > 0:
#                 _epoch_positives_assigned = True
#             _all_vfl: List[float] = []
...
#         (
#             _step_total, _step_o2m, _step_o2o,
#             _step_finite_t, _step_raw_finite_t, _step_diag_t,
#             _step_grad_stats_t,
#         ) = _train_step(
...
#         # The honest NaN signal — head outputs before any sanitiser touched
#         # them.  `_loss_nan_f` above cannot report one (see the accumulator
#         # definitions), so this is what the in-epoch abort is keyed on.
#         _raw_bad_f = 1.0 - tf.cast(_step_raw_finite_t, tf.float32)
#         _acc_raw_nonfinite_count.assign_add(_raw_bad_f)
#         # ── Raw-gradient stats (pre-sanitise, pre-clip, pre-apply) ───────────
#         # Device ops only.  The latch freezes the FIRST step whose raw
#         # gradients went non-finite, which is the step that actually explains
#         # the run — later steps inherit the damage.
#         _grad_stats_last.assign(_step_grad_stats_t)
#         _grad_bad_f = tf.cast(_step_grad_stats_t[_GS_NBAD] > 0.0, tf.float32)
#         # With YOLO_PRO_GRAD_DIAG=0 every field is the -1 sentinel, so the
#         # comparison above is already False and nothing latches.
#         _g_capture  = _grad_bad_f * (1.0 - _grad_bad_captured)
#         _g_keep     = 1.0 - _g_capture
#         _grad_stats_first_bad.assign(
#             _grad_stats_first_bad * _g_keep + _step_grad_stats_t * _g_capture
#         )
#         _grad_bad_step.assign(
#             _grad_bad_step * _g_keep + float(global_step) * _g_capture
#         )
#         _grad_bad_captured.assign(tf.maximum(_grad_bad_captured, _g_capture))
#         # Latch the FIRST raw non-finite step's diagnostics.  Pure device ops
#         # (a select expressed as a blend) so this costs no host sync: once
#         # `_dbg_nan_captured` is 1.0 the blend weight is 0 and the latch is
#         # frozen at the first occurrence.
#         _capture_f = _raw_bad_f * (1.0 - _dbg_nan_captured)
#         _keep_f    = 1.0 - _capture_f
#         _dbg_nan_vec.assign(_dbg_nan_vec * _keep_f + _step_diag_t * _capture_f)
#         _dbg_nan_step.assign(
#             _dbg_nan_step * _keep_f + float(global_step) * _capture_f
#         )
#         _dbg_nan_lr.assign(
#             _dbg_nan_lr * _keep_f
#             + tf.cast(optimizer.learning_rate, tf.float32) * _capture_f
#         )
#         _dbg_nan_captured.assign(tf.maximum(_dbg_nan_captured, _capture_f))
#         # Per-step gradient log (opt-in): forces a host sync every step, which
#         # is exactly what the on-device accumulators exist to avoid — hence the
#         # flag.  Worth it while diagnosing: it is the only view that shows the
#         # gradients of EVERY step rather than one in `_LOSS_READBACK_EVERY`.
#         if _GRAD_DEBUG:
#             _gs = _step_grad_stats_t.numpy()
#             logger.info(
#                 f"[{job_id}] EP{epoch+1} step {_epoch_step+1} "
#                 f"(global {global_step}): "
#                 f"{_format_grad_stats(_gs, _grad_var_names)}"
#                 f"{_loss_scale_report(optimizer)}"
#             )
...
#         # Read the device counters back only every `_LOSS_READBACK_EVERY` steps
#         # for logging / the non-finite abort check — not every iteration.  All
#         # four values (three counters + the BN finiteness flag) are stacked into
#         # ONE tensor so the readback costs a single device→host sync, the same
#         # pattern the old epoch-start BN probe used.
#         if _epoch_step % _LOSS_READBACK_EVERY == 0:
#             _probe = tf.stack([
#                 _acc_skip_count,
#                 _acc_raw_nonfinite_count,
#                 _acc_loss_nan_count,
#                 tf.cast(_bn_finite_flag(), tf.float32),
#             ]).numpy()
#             _skipped  = int(_probe[0])
#             _raw_nan  = int(_probe[1])
#             _loss_nan = int(_probe[2])
#             _bn_ok    = bool(_probe[3])
#             # Dynamic loss scale (MP path only; empty string on fp32/CPU).
#             # Reported on every skip message so a skipped step can be read for
#             # what it is: `halving` = the LossScaleOptimizer calibrating,
#             # `pinned` = gradients non-finite at any representable scale.
#             _scale_now  = _read_loss_scale(optimizer)
#             _scale_note = _loss_scale_report(optimizer, _prev_loss_scale)
#             # Raw-gradient stats for the most recent step, plus the latched
#             # first-bad step.  One extra host sync per readback.
#             _gs_probe = tf.stack([
#                 _grad_stats_last,
#                 _grad_stats_first_bad,
#                 tf.concat(
#                     [tf.stack([_grad_bad_captured, _grad_bad_step]),
#                      tf.zeros([_GRAD_STAT_LEN - 2], dtype=tf.float32)],
#                     axis=0,
#                 ),
#             ]).numpy()
#             _gs_last, _gs_first_bad = _gs_probe[0], _gs_probe[1]
#             _gs_captured = bool(_gs_probe[2][0] > 0.0)
#             _gs_bad_step = int(_gs_probe[2][1])
#             _grad_note = _format_grad_stats(_gs_last, _grad_var_names)

#             # ── Dead loss scale ⇒ abort (recovery is impossible) ─────────────
#             # A LossScaleOptimizer halves on every overflow.  If the scale has
#             # decayed to 0 — or below the point where an overflow could still
#             # be a magnitude problem — the non-finiteness is scale-INVARIANT,
#             # i.e. a NaN, and no further halving can clear it.  At exactly 0
#             # the scaled loss is 0, so there is no gradient signal left at all.
#             # Aborting here replaces the observed behaviour of skipping every
#             # step forever from EP11 onward.
#             if _scale_now is not None and _scale_now <= _MIN_VIABLE_LOSS_SCALE:
#                 _dead = "0 (no gradient signal remains)" if _scale_now == 0.0 \
#                     else f"{_scale_now:g} (below the 2^-16 viability floor)"
#                 raise RuntimeError(
#                     f"[{job_id}] Aborting: the mixed-precision loss scale has "
#                     f"collapsed to {_dead} at EP{epoch+1} step {_epoch_step}. "
#                     f"The LossScaleOptimizer halves once per overflowing step; "
#                     f"reaching this value means the gradients were non-finite "
#                     f"at EVERY scale tried, which a magnitude overflow cannot "
#                     f"be — a NaN is invariant under scaling. This is a NaN "
#                     f"born in the BACKWARD pass, not an fp16 overflow, and it "
#                     f"is why the forward signals all read clean (raw "
#                     f"non-finite head outputs={_raw_nan}, non-finite "
#                     f"total_loss={_loss_nan}, BN moving stats finite={_bn_ok}). "
#                     f"current_scale={_scale_now:g} "
#                     f"previous_scale="
#                     f"{'n/a' if _prev_loss_scale is None else f'{_prev_loss_scale:g}'} "
#                     f"optimizer_iteration={_optimizer_iteration(optimizer)} "
#                     f"skipped_steps_this_epoch={_skipped}. "
#                     f"Last step: {_grad_note}. "
#                     + (
#                         f"First non-finite gradient was at global step "
#                         f"{_gs_bad_step}: "
#                         f"{_format_grad_stats(_gs_first_bad, _grad_var_names)}. "
#                         if _gs_captured else
#                         "No non-finite gradient was ever latched, which is "
#                         "itself a contradiction worth reporting. "
#                     )
#                     + f"Re-run with YOLO_PRO_CHECK_NUMERICS=1 to name the "
#                       f"exact op. {_first_nan_diagnostic()}"
#                 )

#             # First non-finite RAW gradient: logged once per run, as soon as
#             # the latch trips.  This is the report that makes a backward-pass
#             # NaN visible at all — every forward signal can read clean while
#             # it happens.
#             if _gs_captured and not _grad_bad_logged:
#                 _grad_bad_logged = True
#                 _gmsg = (
#                     f"EP{epoch+1} step {_epoch_step}: FIRST non-finite RAW "
#                     f"gradient at global step {_gs_bad_step} — "
#                     f"{_format_grad_stats(_gs_first_bad, _grad_var_names)}"
#                     f"{_scale_note}. Measured before sanitisation, clipping "
#                     f"and unscaling. A non-finite gradient with finite head "
#                     f"outputs and a finite loss means the NaN was created in "
#                     f"the backward pass; re-run with "
#                     f"YOLO_PRO_CHECK_NUMERICS=1 to name the op."
#                 )
#                 logger.warning(f"[{job_id}] {_gmsg}")
#                 _yolo_log_lines.append(_gmsg)
...
#                     f"step(s) so far this epoch — weight update skipped "
#                     f"(true no-op), EMA update suppressed "
#                     f"(raw non-finite head outputs={_raw_nan}; "
#                     f"post-sanitiser non-finite total_loss={_loss_nan} — the "
#                     f"loss sanitizers make this last figure structurally 0)"
#                     f"{_scale_note}  {_grad_note}"
#                 )
...
#                     db.rollback()
#             elif (
#                 _scale_now is not None
#                 and _prev_loss_scale is not None
#                 and _scale_now != _prev_loss_scale
#             ):
#                 # No NEW skips, but the scale moved — this is how the log shows
#                 # the controller doubling back up after calibration, which is
#                 # the evidence that the chosen `initial_scale` is not leaving
#                 # the run underflowed at a floor value.
#                 _scale_msg = (
#                     f"EP{epoch+1} step {_epoch_step}: mixed-precision"
#                     f"{_scale_note}"
#                 )
#                 logger.info(f"[{job_id}] {_scale_msg}")
#                 _yolo_log_lines.append(_scale_msg)
#             if _scale_now is not None:
#                 _prev_loss_scale = _scale_now
#             # First-NaN origin: logged once per run, as soon as the raw probe
#             # trips — well before the abort, so the evidence is in the log even
#             # if the run recovers and no abort ever fires.
#             if _raw_nan > _raw_nonfinite_logged:
#                 _raw_nonfinite_logged = _raw_nan
#                 if not _first_nan_logged:
#                     _first_nan_logged = True
#                     _origin_msg = (
#                         f"EP{epoch+1} step {_epoch_step}: "
#                         f"{_first_nan_diagnostic()}"
#                     )
#                     logger.warning(f"[{job_id}] {_origin_msg}")
#                     _yolo_log_lines.append(_origin_msg)
#             # BN moving stats are updated inside the training=True forward pass,
#             # before gradients exist, so the per-step gradient gate cannot
#             # protect them.  Catch the poisoning HERE rather than at the start
#             # of the next epoch.
#             if not _bn_ok:
#                 raise RuntimeError(
#                     f"[{job_id}] EP{epoch+1} step {_epoch_step}: BN variable "
#                     f"'{_bn_poisoned_variable_name()}' has non-finite moving "
#                     f"stats — model is NaN-poisoned.  Aborting mid-epoch "
#                     f"(raw non-finite steps so far={_raw_nan}, "
#                     f"skipped steps={_skipped}).{_scale_note}  "
#                     f"{_first_nan_diagnostic()}"
#                 )
#             if _raw_nan >= _MAX_NAN_STEPS_PER_EPOCH:
#                 raise RuntimeError(
#                     f"[{job_id}] Aborting: {_raw_nan} training steps in "
#                     f"EP{epoch+1} produced non-finite RAW head outputs "
#                     f"(threshold={_MAX_NAN_STEPS_PER_EPOCH}).{_scale_note} "
#                     f"Last finite mean_loss before NaN run: "
#                     f"{last_finite_mean_loss}. Numerical instability in the "
#                     f"loss/gradient path — not a checkpoint-selection issue. "
#                     f"{_first_nan_diagnostic()}. Note: the weight update is "
#                     f"skipped on such steps, but BN moving stats may be "
#                     f"partially poisoned by the non-finite forward passes."
#                 )
...
#         # Perf: `_bn_finite_flag()` reduces every BN moving-stat variable to a
#         # scalar ON-DEVICE and stacks them, so this costs a SINGLE `.numpy()`
#         # host sync; the per-variable scan that names the offender only runs on
#         # the cold NaN-poisoned path.  The same probe now ALSO runs at the
#         # in-epoch readback cadence (see `_consume_batch`), so this epoch-start
#         # call is the backstop rather than the only line of defence.
#         if not bool(_bn_finite_flag().numpy()):
#             raise RuntimeError(
#                 f"[{job_id}] EP{epoch+1} start: BN variable "
#                 f"'{_bn_poisoned_variable_name()}' has non-finite moving "
#                 f"stats — model is NaN-poisoned. Aborting."
#             )
...
#         _acc_raw_nonfinite_count.assign(0.0)
#         # Host-side bookkeeping for the periodic readback / logging.
#         _epoch_first_global_step = global_step   # for the warmup-window ratio
#         _epoch_step: int = 0            # step index within this epoch
#         _skip_logged: int = 0           # skipped-step count already logged
#         _raw_nonfinite_logged: int = 0  # raw non-finite count already logged
#         # Post-sanitiser non-finite total_loss count.  Reporting only — the
#         # sanitizers make it structurally 0; the abort is keyed on the RAW
#         # counter (`_acc_raw_nonfinite_count`).
#         nan_step_count_this_epoch: int = 0
#         # Set by the first-batch TAL diagnostic when the assigner produced at
#         # least one positive this epoch.  Consumed by the checkpoint sanity
#         # gate, where val_cls_pos == 0.0 is only meaningful if there WERE
#         # positives to score.
#         _epoch_positives_assigned: bool = False
...
#         # The four counters are stacked so they cost a SINGLE device→host sync
#         # (the same pattern `_consume_batch` uses for its in-epoch readback).
#         _epoch_probe = tf.stack([
#             _acc_finite_count,
#             _acc_skip_count,
#             _acc_loss_nan_count,
#             _acc_raw_nonfinite_count,
#         ]).numpy()
#         _finite_count   = int(_epoch_probe[0])
#         _skipped_final  = int(_epoch_probe[1])
#         nan_step_count_this_epoch = int(_epoch_probe[2])
#         _raw_nonfinite_final = int(_epoch_probe[3])
#         _attempted_steps = _finite_count + _skipped_final
#         _skip_ratio = (
#             _skipped_final / _attempted_steps if _attempted_steps > 0 else 0.0
#         )
#         # ── What a skipped step MEANS, per optimizer ────────────────────────
#         # fp32/CPU (plain AdamW): the `tf.cond` gate suppressed a genuinely
#         #   non-finite update — something is wrong, and the strict skip-ratio
#         #   abort below is the guard that caught the 13-of-16 poisoning.
#         # MP/GPU (LossScaleOptimizer): the SCALED gradients overflowed float16,
#         #   so the optimizer declined to apply the step and halved the dynamic
#         #   scale.  That is the controller working as designed; during the
#         #   initial calibration chain it is the expected outcome of every step,
#         #   and the weights it declined to touch are exactly as healthy as they
#         #   were before the epoch.
#         # So on the MP path the abort is keyed on the honest corruption signals
#         # — raw (pre-sanitiser) non-finite head outputs, a non-finite
#         # total_loss, or poisoned BN moving stats — and a high skip ratio with
#         # none of them present is reported as a warning and allowed to proceed.
#         _loss_scaled   = _is_loss_scale_optimizer(optimizer)
#         _scale_now     = _read_loss_scale(optimizer)
#         _scale_report  = _loss_scale_report(optimizer, _prev_loss_scale)
#         _scale_falling = (
#             _scale_now is not None and _prev_loss_scale is not None
#             and _scale_now < _prev_loss_scale
#         )
#         if _scale_now is not None:
#             _prev_loss_scale = _scale_now
#         # BN moving stats are updated inside the `training=True` forward pass,
#         # so they are poisoned independently of whether any step was applied —
#         # the third honest signal, and the one a skipped step cannot protect.
#         _bn_finite_now = bool(_bn_finite_flag().numpy())
#         # ── Raw-gradient evidence for the epoch ─────────────────────────────
#         # The signal that was missing entirely.  A gradient can be non-finite
#         # while every forward signal above reads clean, and the distinction
#         # between the two kinds matters:
#         #   • Inf  — consistent with a genuine float16 overflow; halving the
#         #            loss scale can fix it, so under MP this is benign.
#         #   • NaN  — invariant under scaling.  No amount of halving clears it,
#         #            so a NaN that persists as the scale falls is a defect in
#         #            the backward graph, not a magnitude problem.  The
#         #            `_MIN_VIABLE_LOSS_SCALE` abort in the readback is what
#         #            turns that into a hard stop.
#         _eg_probe = tf.stack([
#             _grad_stats_last,
#             _grad_stats_first_bad,
#             tf.concat(
...
#             ),
#         ]).numpy()
#         _eg_last, _eg_first_bad = _eg_probe[0], _eg_probe[1]
#         _eg_captured = bool(_eg_probe[2][0] > 0.0)
#         _eg_bad_step = int(_eg_probe[2][1])
#         _eg_note = f"  last-step {_format_grad_stats(_eg_last, _grad_var_names)}"
#         _eg_first_note = (
#             f" First non-finite gradient this run was at global step "
#             f"{_eg_bad_step}: "
#             f"{_format_grad_stats(_eg_first_bad, _grad_var_names)}."
#             if _eg_captured else ""
#         )
#         _grad_nan_seen = float(_eg_last[_GS_NAN_IDX]) >= 0.0 or (
#             _eg_captured and float(_eg_first_bad[_GS_NAN_IDX]) >= 0.0
#         )
#         _epoch_corrupted = (
#             _raw_nonfinite_final > 0
#             or nan_step_count_this_epoch > 0
#             or not _bn_finite_now
#         )
#         # Steps of this epoch that fell inside the loss-scale warmup window.
#         # The longest possible calibration chain is log2(initial_scale)
#         # consecutive skips, so those steps cannot be evidence of anything and
#         # are excluded from the ratio the guard judges — without this a 19-step
#         # epoch can never satisfy a 25 % ceiling under MP, because warmup alone
#         # needs more skips than the ceiling allows.  On the fp32 path the
#         # window is empty and the judged ratio is exactly the ratio the guard
#         # has always used.
#         _warmup_overlap = (
#             max(0, min(_mp_warmup_steps, global_step) - _epoch_first_global_step)
#             if _loss_scaled else 0
#         )
#         _judged_steps = max(0, _attempted_steps - _warmup_overlap)
#         _judged_skips = max(0, _skipped_final - _warmup_overlap)
#         _judged_ratio = (
#             _judged_skips / _judged_steps if _judged_steps > 0 else 0.0
#         )
#         _warmup_note = (
#             f" ({_warmup_overlap} of these fell inside the {_mp_warmup_steps}"
#             f"-step loss-scale warmup window and are excluded from the guard "
#             f"ratio {_judged_skips}/{_judged_steps}={_judged_ratio:.0%})"
#             if _warmup_overlap > 0 else ""
#         )
#         if _skipped_final > _skip_logged:
#             _skip_logged = _skipped_final
#             _nan_msg = (
#                 f"EP{epoch+1}: {_skipped_final}/{_attempted_steps} non-finite "
#                 f"step(s) this epoch ({_skip_ratio:.0%}) — weight update "
#                 f"skipped (true no-op), EMA suppressed "
#                 f"(raw non-finite head outputs={_raw_nonfinite_final}; "
#                 f"post-sanitiser non-finite total_loss="
#                 f"{nan_step_count_this_epoch} — structurally 0 while the loss "
#                 f"sanitizers are in place)"
#                 f"{_scale_report}{_warmup_note}{_eg_note}"
#             )
#             logger.warning(f"[{job_id}] {_nan_msg}")
#             _yolo_log_lines.append(_nan_msg)
#         if _finite_count == 0:
#             if _loss_scaled and not _epoch_corrupted:
#                 # NOT corruption — and the old message ("every step skipped as
#                 # non-finite — no valid loss to report") implied it was.  Every
#                 # forward pass was clean (raw non-finite == 0), every
#                 # total_loss was finite, BN stats are intact, and a
#                 # LossScaleOptimizer skips a step by NOT applying it — so no
#                 # weight was modified.  The dynamic scale merely failed to
#                 # finish calibrating inside this epoch.
#                 _cal_msg = (
#                     f"EP{epoch+1}: loss-scale calibration did not converge "
#                     f"within the epoch — all {_attempted_steps} step(s) "
#                     f"overflowed float16 and were skipped by the "
#                     f"LossScaleOptimizer, so no gradient was applied and this "
#                     f"epoch has no mean loss to report. The model is intact: "
#                     f"raw non-finite head outputs=0, total_loss finite on "
#                     f"every step, BN moving stats finite."
#                     f"{_scale_report}.{_eg_note}.{_eg_first_note}"
#                     + (
#                         # A NaN in the gradients is scale-invariant, so calling
#                         # this "calibration" would be wrong: halving cannot
#                         # clear it and the scale will run to the viability
#                         # floor, where the readback guard aborts.
#                         " WARNING: the raw gradients contain NaN (not merely "
#                         "Inf). NaN is invariant under loss scaling, so this "
#                         "will NOT resolve by halving — expect the scale to "
#                         "decay to the viability floor and abort. Re-run with "
#                         "YOLO_PRO_CHECK_NUMERICS=1 to name the backward op."
#                         if _grad_nan_seen else
#                         f" Continuing — the calibrated scale carries into "
#                         f"EP{epoch+2}" + (
#                             "." if _scale_falling else
#                             "; NOTE the scale is NOT descending, so if this "
#                             "repeats the gradients are likely non-finite at "
#                             "any scale."
#                         )
#                     )
#                 )
#                 logger.warning(f"[{job_id}] {_cal_msg}")
#                 _yolo_log_lines.append(_cal_msg)
#                 # No applied step means no loss to average.  NaN (not 0.0) so
#                 # the epoch cannot win checkpoint selection or move best_loss,
#                 # and `_sanitize_json` renders it as null in training_history.
#                 mean_loss = float("nan")
#                 mean_o2m  = float("nan")
#                 mean_o2o  = float("nan")
#             else:
#                 # fp32/CPU, or MP with a real corruption signal: the raw
#                 # non-finite threshold should have fired first.
#                 raise RuntimeError(
#                     f"[{job_id}] EP{epoch+1}: every step skipped as non-finite "
#                     f"— no valid loss to report (raw non-finite head "
#                     f"outputs={_raw_nonfinite_final}, non-finite "
#                     f"total_loss={nan_step_count_this_epoch}, BN moving stats "
#                     f"finite={_bn_finite_now}).{_scale_report}{_eg_note}."
#                     f"{_eg_first_note} {_first_nan_diagnostic()}"
#                 )
#         else:
#             if _judged_ratio > _MAX_SKIPPED_STEP_RATIO_PER_EPOCH:
#                 if _loss_scaled and not _epoch_corrupted:
#                     # High skip ratio with a clean forward pass on every step:
#                     # the scale controller is still hunting for a workable
#                     # scale.  Warn and proceed — aborting here is what killed
#                     # job 8718cdaf, whose weights were never touched.
#                     _warn_msg = (
#                         f"EP{epoch+1}: high skipped-step ratio "
#                         f"{_judged_skips}/{_judged_steps} "
#                         f"({_judged_ratio:.0%} > "
#                         f"{_MAX_SKIPPED_STEP_RATIO_PER_EPOCH:.0%}) under mixed "
#                         f"precision — but every raw head output was finite "
#                         f"(raw non-finite=0), total_loss was finite on every "
#                         f"step and BN moving stats are clean, so this is "
#                         f"dynamic loss-scale calibration, not model "
#                         f"corruption.{_scale_report}{_eg_note}. Proceeding "
#                         f"on the {_finite_count} applied step(s)"
#                         + (
#                             " — WARNING: the raw gradients contain NaN, which "
#                             "loss scaling cannot clear; this will decay to the "
#                             "viability floor and abort."
#                             if _grad_nan_seen else
#                             "" if _scale_falling else
#                             " — watch the scale: it is not descending"
#                         )
#                     )
#                     logger.warning(f"[{job_id}] {_warn_msg}")
#                     _yolo_log_lines.append(_warn_msg)
#                 else:
#                     # A model that could not take most of its gradient steps
#                     # must not proceed to validation and checkpoint admission:
#                     # the epoch's few surviving steps produce a mean_loss that
#                     # looks reportable while the weights are effectively frozen
#                     # (or already poisoned).  The older guard only fired at a
#                     # 100 % skip rate, which let an epoch with 13 of 16 steps
#                     # skipped admit a NaN-poisoned ★ckpt.
#                     raise RuntimeError(
#                         f"[{job_id}] Aborting: EP{epoch+1} skipped "
#                         f"{_skipped_final}/{_attempted_steps} steps as "
#                         f"non-finite ({_judged_ratio:.0%} of the "
#                         f"{_judged_steps} judged step(s) > "
#                         f"{_MAX_SKIPPED_STEP_RATIO_PER_EPOCH:.0%} threshold) — "
#                         f"the model is not training. Raw non-finite head "
#                         f"outputs this epoch={_raw_nonfinite_final}, "
#                         f"non-finite total_loss={nan_step_count_this_epoch}, "
#                         f"BN moving stats finite={_bn_finite_now}."
#                         f"{_scale_report}{_eg_note}.{_eg_first_note} "
#                         f"Last finite mean_loss: {last_finite_mean_loss}. "
#                         f"{_first_nan_diagnostic()}"
#                     )
...
#             f"loss={mean_loss:.4f}"
#             # Empty on the fp32/CPU path; under MP this is the only place the
#             # dynamic scale is visible on a healthy epoch, which is how the
#             # log shows it growing back after calibration.
#             f"{_scale_report}"
#         )
...
#         # Phase D: save on monitor improvement, gated by the loss-side cls
#         # ranking guard (``_ckpt_cls_guard_passed``, computed above from
#         # val_cls vs. the saved checkpoint's val_cls) AND the model-sanity gate
#         # below.  The real-inference safety gate and both rescue paths were
#         # removed; all deployment-safety evaluation is deferred to the single
#         # post-training finalist eval, which re-scores every buffered
#         # checkpoint on the full validation set and refuses to export a
#         # collapsed one.
#         # ── Model-sanity gate (runs BEFORE anything enters the EMA export
#         # buffer) ─────────────────────────────────────────────────────────
#         # A NaN-poisoned model can still produce a numerically "improving"
#         # monitor value and slip through the cls ranking guard: the observed
#         # failure admitted a ★ckpt at val_loss=790.4522 with
#         # val_cls_pos=0.0000 / val_cls_neg=196.9461.  val_cls_pos == 0.0
#         # EXACTLY means the positive branch of VFL contributed nothing at all
#         # — with positives assigned during training that is impossible for a
#         # live model (it requires every positive cell to score a perfect 1.0)
#         # and is the signature of a collapsed / poisoned head whose scores were
#         # substituted by the loss sanitizers.  These three checks are cheap and
#         # unambiguous; ranking quality remains the job of the post-training
#         # finalist eval.
#         _ckpt_sanity_failures: List[str] = []
#         if mean_val_loss is not None and not math.isfinite(float(mean_val_loss)):
#             _ckpt_sanity_failures.append(
#                 f"val_loss={mean_val_loss} is non-finite"
#             )
#         if (
#             mean_val_cls_pos is not None
#             and float(mean_val_cls_pos) == 0.0
#             and _epoch_positives_assigned
#         ):
#             _ckpt_sanity_failures.append(
#                 "val_cls_pos == 0.0 exactly while TAL assigned positives "
#                 "during training — the positive branch contributed nothing, "
#                 "which indicates a collapsed or NaN-poisoned cls head"
#             )
#         if not bool(_bn_finite_flag().numpy()):
#             _ckpt_sanity_failures.append(
#                 f"BN moving stats are non-finite "
#                 f"(first offender: {_bn_poisoned_variable_name()})"
#             )
#         _ckpt_sanity_ok = not _ckpt_sanity_failures

#         _save_ckpt = (
#             _ckpt_monitor_improved
#             and _ckpt_cls_guard_passed
#             and _ckpt_sanity_ok
#         )
#         _ckpt_guard_rejected = _ckpt_monitor_improved and not _ckpt_cls_guard_passed
#         _ckpt_sanity_rejected = (
#             _ckpt_monitor_improved and _ckpt_cls_guard_passed
#             and not _ckpt_sanity_ok
#         )
...
#         if _ckpt_sanity_rejected:
#             # Monitor improved and the cls guard passed, but the model itself
#             # is not sane.  Patience is NOT held here (unlike the cls-guard
#             # path): this is not a recoverable calibration wobble, and burning
#             # patience is the desired behaviour if it somehow persists.
#             logger.warning(
#                 f"[{job_id}] EP{epoch+1} checkpoint REJECTED (model sanity) — "
#                 f"{ckpt_metric_name}={monitor_loss:.4f} improved but the "
#                 f"candidate failed: {'; '.join(_ckpt_sanity_failures)}.  "
#                 f"Not admitted to the EMA export buffer."
#             )
...
#                 ),
#                 "failed_gates":     [
#                     f"model_sanity: {_f}" for _f in _ckpt_sanity_failures
#                 ],
#                 "val_cls":            mean_val_cls,
...
#                     "full_background_fp_rate":      None,
#                     "full_background_images":       None,
...
#             _entry["full_background_fp_rate"]      = None
#             _entry["full_background_images"]       = None
...
#         # Background false-positive rate feeds the leading gate tier of
#         # ``_export_candidate_rank_key``.  Stays None when the test split has
#         # no GT-free images, which is the gate's pass-through case.
#         _entry["full_background_fp_rate"]      = _finite(_rep.get("background_fp_rate"))
#         _entry["full_background_images"]       = _rep.get("background_images")
...
#             return "None"
#         # Background-FP gate verdict for this finalist.  Reported per-finalist
#         # (not only for the winner) so a demotion is visible in the log that
#         # explains the selection, rather than showing up as an unexplained
#         # lower-mAP checkpoint being shipped.
#         _bg_rate = _entry.get("full_background_fp_rate")
#         _bg_tag = ""
#         if isinstance(_bg_rate, (int, float)) and math.isfinite(float(_bg_rate)):
#             _bg_tag = (
#                 f" bg_fp_rate={float(_bg_rate):.3f}/img over "
#                 f"{_rep.get('background_images')} background image(s) "
#                 f"[gate {'FAIL' if float(_bg_rate) > _CKPT_BG_FP_RATE_MAX else 'pass'} "
#                 f"@ {_CKPT_BG_FP_RATE_MAX:.2f}]"
#             )
#         _mt_line = (
#             f"[{job_id}] Model Testing: finalist {_rank + 1}/{len(_eval_order)} "
...
#     # ── Background-FP gate outcome ───────────────────────────────────────────
#     # Report only when the gate actually did something: at least one finalist
#     # carries a measured rate AND at least one is over the ceiling.  On a
#     # dataset with no background images every rate is None, nothing is logged,
#     # and selection above already ran the pre-gate ranking unchanged.
#     _bg_rated = [
#         _e for _e in _ok_entries
#         if isinstance(_e.get("full_background_fp_rate"), (int, float))
#         and math.isfinite(float(_e.get("full_background_fp_rate")))
#     ]
#     _bg_failed = [
#         _e for _e in _bg_rated
#         if float(_e["full_background_fp_rate"]) > _CKPT_BG_FP_RATE_MAX
#     ]
#     if _bg_failed:
#         _bg_sel_rate = _restored_entry.get("full_background_fp_rate")
#         _bg_sel_ok = not (
#             isinstance(_bg_sel_rate, (int, float))
#             and math.isfinite(float(_bg_sel_rate))
#             and float(_bg_sel_rate) > _CKPT_BG_FP_RATE_MAX
#         )
#         _bg_line = (
#             f"Background-FP gate: {len(_bg_failed)}/{len(_bg_rated)} scored "
#             f"finalist(s) exceeded {_CKPT_BG_FP_RATE_MAX:.2f} detections per "
#             f"background image and were demoted below the passing candidates "
#             f"(epochs {sorted(int(_e['epoch']) for _e in _bg_failed if _e.get('epoch') is not None)}). "
#             + (
#                 "The shipped checkpoint passes the gate."
#                 if _bg_sel_ok else
#                 "NO finalist passed the gate — the least-bad candidate was "
#                 "shipped; expect false positives on empty frames and consider "
#                 "adding background images or raising the deployment threshold."
#             )
#         )
#         logger.info(f"[{job_id}] {_bg_line}")
#         _yolo_log_lines.append(_bg_line)
...
#             "full_background_fp_rate":      _e.get("full_background_fp_rate"),
#             "full_background_images":       _e.get("full_background_images"),
#         }
#         for _e in checkpoint_manifest
#     ]
...
import numpy as np
import tensorflow as tf
# DEBUG INSTRUMENTATION ONLY — every call is a no-op unless YOLO_PRO_NAN_PROBE=1
# (see app/ml/yolo_pro/_debug_probe.py).  Never referenced by training math.
from app.ml.yolo_pro._debug_probe import (
    probe as _dbg,
    probe_scalar as _dbgs,
    probe_grads as _dbgg,
    probe_weights as _dbgw,
    set_ctx as _dbg_ctx,
    set_tag as _dbg_tag,
)
from datetime import datetime
...
def _tal_score_boxes(
    ...
    for box in boxes:
        ...
        if pred_cls_np is not None and pred_boxes_np is not None:
            ...
            iou_vals = _box_iou_matrix(pb, gt_n)[:, 0]                         # (K,)
            iou_vals = np.clip(iou_vals, 0., 1.)
            # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1)
            _dbg("03_TAL", "tal.cls_score", cls_score, note=f"stride={stride}")
            _dbg("03_TAL", "tal.pred_boxes_cand", pb, note=f"stride={stride}")
            _dbg("03_TAL", "tal.iou_vals", iou_vals, note=f"stride={stride}")
            tal_score   = np.maximum((cls_score ** alpha) * (iou_vals ** beta), 1e-8)
            vfl_quality = iou_vals
        else:
            # ── Static geometric fallback ─────────────────────────────────────
        ...
        # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1) - per-GT TAL internals.
        _dbgs("03_TAL", "tal.n_candidates", len(inside_idx),
              note=f"stride={stride} N={N} dyn={pred_cls_np is not None}")
        _dbgs("03_TAL", "tal.gt_box_px_area", (x2 - x1) * (y2 - y1),
              note=f"x1={x1:.2f} y1={y1:.2f} x2={x2:.2f} y2={y2:.2f} cls={cls_idx}")
        _dbg("03_TAL", "tal.tal_score", tal_score, note=f"stride={stride}")
        _dbg("03_TAL", "tal.vfl_quality", vfl_quality, note=f"stride={stride}")
        scored.append({
            "cls_idx":     cls_idx,
            "inside_idx":  inside_idx,
            "tal_score":   tal_score,
...
def _resolve_targets(
    ...
    for rec in scored:
        ...
        for local_j, anchor_j in enumerate(inside_idx):
            ...
            ltrb_targets[anchor_j] = np.clip(ltrb, 0.0, reg_max - 1e-3)
            fg_mask[anchor_j]      = True
            # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1) - how much of the
            # raw ltrb distance survives the [0, reg_max) clamp for this anchor.
            _dbgs("03_TAL", "tal.ltrb_raw_max", float(np.max(ltrb)),
                  note=f"stride={stride} topk={topk} "
                       f"clipped={int(np.sum(ltrb > (reg_max - 1e-3)))}/4")

    # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1) - resolved targets.
    _dbg("03_TAL", "assign.cls_targets", cls_targets, note=f"stride={stride} topk={topk}")
    _dbg("03_TAL", "assign.box_targets", box_targets, note=f"stride={stride} topk={topk}")
    _dbg("03_TAL", "assign.ltrb_targets", ltrb_targets, note=f"stride={stride} topk={topk}")
    _dbg("03_TAL", "assign.fg_mask", fg_mask, note=f"stride={stride} topk={topk}")
    return cls_targets, box_targets, ltrb_targets, fg_mask
...
def _decode_reg_scale(
    ...
    ltrb_grid = tf.reduce_sum(probs * bins, axis=-1)  # (B, N, 4) in grid units
    ltrb_px   = ltrb_grid * tf.cast(stride, tf.float32)  # → pixels
    # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1) - DFL decode internals.
    _dbg("04_DECODE", "decode.reg_pred_sanitized", reg_pred)
    _dbg("04_DECODE", "decode.probs", probs)
    _dbg("04_DECODE", "decode.ltrb_grid", ltrb_grid)
    _dbg("04_DECODE", "decode.ltrb_px", ltrb_px)
    _dbgs("04_DECODE", "decode.stride", stride)
    ...
    # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1)
    _dbg("04_DECODE", "decode.pred_boxes", pred_boxes)
    _dbg("04_DECODE", "decode.pred_w", pred_boxes[..., 2] - pred_boxes[..., 0])
    _dbg("04_DECODE", "decode.pred_h", pred_boxes[..., 3] - pred_boxes[..., 1])
    return pred_boxes
...
def _train_step_impl(
    ...
    # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1) - stage 01: input tensors,
    # loss weights, LR, and the weights this step starts from.
    _dbg_tag("step")
    _dbg("01_INPUT", "images", images,
         note=f"H_img={H_img} W_img={W_img} reg_max={reg_max}")
    _dbgs("01_INPUT", "w_cls", w_cls)
    _dbgs("01_INPUT", "w_box", w_box)
    _dbgs("01_INPUT", "w_dfl", w_dfl)
    _dbgs("01_INPUT", "w_o2o", w_o2o)
    _dbgs("01_INPUT", "learning_rate", optimizer.learning_rate)
    _dbgw("09_WEIGHTS_PRE", model.trainable_variables, note="before apply_gradients")

    with tf.GradientTape() as tape:
        preds = model(images, training=True)
        # DEBUG PROBE - stage 02: raw head outputs, per scale, as produced.
        for _sn in ("p3", "p4", "p5"):
            _dbg("02_MODEL_OUT", f"raw_cls_{_sn}", preds[f"cls_{_sn}"])
            _dbg("02_MODEL_OUT", f"raw_reg_{_sn}", preds[f"reg_{_sn}"])
        o2m_total = tf.constant(0.0, dtype=tf.float32)
        o2o_total = tf.constant(0.0, dtype=tf.float32)
        ...
        for (scale_name,
            ...
            cls_pred = tf.cast(cls_pred, tf.float32)
            reg_pred = tf.cast(reg_pred, tf.float32)
            # DEBUG PROBE - reshaped/cast head outputs entering the loss.
            _dbg_tag(f"{scale_name}")
            _dbg("02_MODEL_OUT", "cls_pred_f32", cls_pred)
            _dbg("02_MODEL_OUT", "reg_pred_f32", reg_pred)
            ...
            # ── o2m loss (one-to-many, topk=10) ──────────────────────────────
            _dbg_tag(f"o2m.{scale_name}")   # DEBUG PROBE tag only
            o2m_losses = yolo_pro_loss(
                cls_pred=cls_pred,   reg_pred=reg_pred,
                cls_targets=cls_t,   box_targets=box_t,
                ltrb_targets=ltrb_t, fg_mask=fg,
            ...
            # DEBUG PROBE - o2m components for this scale.
            _dbg("07_O2M", "o2m.loss_cls", o2m_losses["loss_cls"])
            _dbg("07_O2M", "o2m.loss_box", o2m_losses["loss_box"])
            _dbg("07_O2M", "o2m.loss_dfl", o2m_losses["loss_dfl"])
            _dbg("07_O2M", "o2m.loss_total", o2m_losses["loss_total"])
            o2m_total = o2m_total + o2m_losses["loss_total"]
            _dbg("07_O2M", "o2m.running_total", o2m_total)

            # ── o2o loss (one-to-one, topk=1) ────────────────────────────────
            _dbg_tag(f"o2o.{scale_name}")   # DEBUG PROBE tag only
            o2o_losses = yolo_pro_loss(
                cls_pred=cls_pred,       reg_pred=reg_pred,
                cls_targets=o2o_cls_t,   box_targets=o2o_box_t,
                ltrb_targets=o2o_ltrb_t, fg_mask=o2o_fg,
            ...
            # DEBUG PROBE - o2o components for this scale.
            _dbg("08_O2O", "o2o.loss_cls", o2o_losses["loss_cls"])
            _dbg("08_O2O", "o2o.loss_box", o2o_losses["loss_box"])
            _dbg("08_O2O", "o2o.loss_dfl", o2o_losses["loss_dfl"])
            _dbg("08_O2O", "o2o.loss_total", o2o_losses["loss_total"])
            o2o_total = o2o_total + o2o_losses["loss_total"]
            _dbg("08_O2O", "o2o.running_total", o2o_total)

        total_loss = o2m_total + tf.cast(w_o2o, tf.float32) * o2o_total
        # DEBUG PROBE - stage 08: the totals actually differentiated.
        _dbg_tag("step")
        _dbg("08_TOTAL", "o2m_total", o2m_total)
        _dbg("08_TOTAL", "o2o_total", o2o_total)
        _dbg("08_TOTAL", "total_loss", total_loss)
        # Mixed-precision loss scaling (Keras 3 API): under a LossScaleOptimizer
        # (GPU/MP path) scale the float32 loss up inside the tape so small
        # gradients survive the float16 backward pass; apply_gradients below then
        # unscales, clips, skips non-finite steps, and adjusts the dynamic scale.
    ...
    _dbg("08_TOTAL", "loss_for_grad", loss_for_grad)   # DEBUG PROBE

    grads = tape.gradient(loss_for_grad, model.trainable_variables)
    # DEBUG PROBE - stage 09: raw gradients straight out of the tape, BEFORE any
    # clipping and before the optimizer's own non-finite handling.
    _dbgg("09_GRAD", grads, model.trainable_variables, note="raw from tape")
    ...
    # DEBUG PROBE - stage 10: exactly what the optimizer is handed.
    _dbg("10_OPT_IN", "step_finite", tf.cast(step_finite, tf.float32))
    _dbg("10_OPT_IN", "grads_finite", tf.cast(grads_finite, tf.float32))
    _dbgs("10_OPT_IN", "opt.learning_rate", optimizer.learning_rate)
    _dbgs("10_OPT_IN", "opt.iterations", optimizer.iterations)
    _dbgg("10_OPT_IN", grads, model.trainable_variables, note="into apply_gradients")

    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    # DEBUG PROBE - stage 11: weights after the update.
    _dbgw("11_WEIGHTS_POST", model.trainable_variables, note="after apply_gradients")
...
def run_yolo_pro_training(
    ...
    def _build_batch(batch_start, batch_indices,
        ...
        imgs_np = np.stack(aug_imgs_list).astype(np.float32)
        _B = len(aug_imgs_list)
        # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1) - stage 00: the batch
        # of augmented input images leaving the producer thread.
        _dbg_ctx(res=f"{H}x{W}", epoch=epoch + 1, batch=batch_start // batch_size)
        _dbg_tag("input")
        _dbg("00_IMAGES", "imgs_np", imgs_np,
             note=f"mosaic_gate={mosaic_epoch_gate} hflip_p={epoch_hflip_prob} "
                  f"jitter_p={epoch_color_jitter_prob}")
        ...
        if _reuse_tal:
            ...
            try:
                ...
                for _sname, (_apt, _s) in zip(["p3", "p4", "p5"], anchor_grids):
                    ...
                    # DEBUG PROBE - dynamic-TAL forward outputs feeding assignment.
                    _dbg("01_TAL_FWD", f"tal_fwd.cls_{_sname}", _cn,
                         note=f"stride={_s} anchors={_apt.shape[0]}")
                    _dbg("01_TAL_FWD", f"tal_fwd.reg_{_sname}", _rn, note=f"stride={_s}")
                    _dbg("01_TAL_FWD", f"tal_fwd.boxes_{_sname}", _dyn_boxes[_sname],
                         note=f"stride={_s}")
                _use_dynamic_tal = True
                # Seed the reuse cache with this fresh forward (Phase 4).  Skipped
                # on the k=1 default so that path retains no extra references.
        ...
        # DEBUG PROBE - stage 03b: the stacked target arrays handed to the step.
        for _tk, _tv in targets.items():
            _dbg("03_TARGETS", f"targets.{_tk}", _tv)
        return batch_start, imgs_np, targets, _use_dynamic_tal
    ...
    def _consume_batch(batch_start, imgs_np, targets, _use_dynamic_tal):
        ...
        optimizer.learning_rate.assign(lr_schedule_fn(global_step))
        # DEBUG PROBE (no-op unless YOLO_PRO_NAN_PROBE=1) - stamp epoch/step/batch
        # on every probe line emitted by this step.
        _dbg_ctx(res=f"{H}x{W}", epoch=epoch + 1, step=global_step,
        ...
        # ── On-device accumulation (Change #1) — no host sync per step ────────
        # DEBUG PROBE - host-side view of the step outcome.
        _dbgs("12_STEP_OUT", "step_total", _step_total)
        _dbgs("12_STEP_OUT", "step_o2m", _step_o2m)
        _dbgs("12_STEP_OUT", "step_o2o", _step_o2o)
        _dbgs("12_STEP_OUT", "step_finite", tf.cast(_step_finite_t, tf.float32))
        _finite_f   = tf.cast(_step_finite_t, tf.float32)
        _loss_nan_f = 1.0 - tf.cast(tf.math.is_finite(_step_total), tf.float32)
        _acc_loss.assign_add(_step_total * _finite_f)
        _acc_o2m.assign_add(_step_o2m * _finite_f)
