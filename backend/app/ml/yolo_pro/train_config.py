...
class TrainConfig:
    ...
    # Top-K disk-backed checkpoint retention (Phase E): the trainer keeps the K
    # best epochs by val_loss as ``.keras`` files on disk; the post-training
    # finalist eval re-scores exactly these K.  Default 8 matches the retired
    # in-RAM ``_BEST_EMA_BUFFER_CAP``.  Lower it to bound disk usage for large
    # backbones / high input resolution (see the disk-budget guard in the
    # worker); raise it for more finalist margin on noisy runs.
    topk_checkpoints: int = 8
    ...
    def __post_init__(self):
        ...
        assert self.topk_checkpoints >= 1, (
            f"topk_checkpoints must be >= 1; got {self.topk_checkpoints}"
        )
    ...
    def build_optimizer(
        ...
        return tf.keras.optimizers.AdamW(
            ...
            beta_2         = 0.999,
            epsilon        = 1e-7,
            # Global-norm gradient clip at 10.0.  Applied inside apply_gradients
            # on the (already unscaled) gradients, so it composes correctly with
            # the mixed-precision LossScaleOptimizer wrapper — replacing the
            # previous manual tf.clip_by_global_norm(grads, 10.0) in _train_step.
...
class ModelEMA:
    ...
    def update(self, model: tf.keras.Model, step_finite=True) -> None:
        """Update shadow weights: ema = decay*ema + (1-decay)*live.

        The per-weight loop is fused into a single ``@tf.function`` graph op
        (``_ema_update_graph``) so the whole update runs as one traced graph
        instead of an eager Python loop that launches a kernel per trainable
        weight every step.

        ``step_finite`` gates the update on-device: when it is False / 0 the
        shadows are left unchanged, so a non-finite training step cannot poison
        the EMA (equivalent to skipping ``ema.update`` on a bad step, but with
        no host<->device sync to read the flag).  The arithmetic runs in each
        shadow's own dtype (float32 for the master weights, even under a
        mixed-precision policy), so accuracy is unaffected.
        """
        self._ema_update_graph(
            model.trainable_weights, tf.cast(step_finite, tf.float32)
        )

    @tf.function
    def _ema_update_graph(self, live_weights, finite_mask) -> None:
        # finite_mask is a scalar in {0., 1.}.  When 1., the assignment reduces
        # exactly to `decay*shadow + (1-decay)*live` (unchanged math); when 0.,
        # `shadow` is written back unchanged (suppressed update).
        for shadow, live in zip(self._shadows, live_weights):
            live_c = tf.cast(live, shadow.dtype)
            new    = self.decay * shadow + (1.0 - self.decay) * live_c
            shadow.assign(shadow + finite_mask * (new - shadow))
